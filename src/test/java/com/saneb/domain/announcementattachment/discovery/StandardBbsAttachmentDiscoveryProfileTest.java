...
class StandardBbsAttachmentDiscoveryProfileTest {
    ...
    static AttachmentDiscoveryProfile.Source selectSourceUrl(Case sample, String url) {
        ...
        return new AttachmentDiscoveryProfile.Source("LOCAL_GOV_NOTICE", normalizer.hash(normalizer.canonicalizeUrl(url)),
                url, sample.sourceCode(), "SPRING_BBS");
    }
    static boolean selectCompact(Case sample) { return sample.board().equals("65"); }
    static String selectPage(Case sample, String items) {
        boolean compact = selectCompact(sample);
        return (compact ? "<div class='p-wrap bbs bbs__view'><table class='p-table block'>" : "<table class='bbs_default view'>")
                + "<tr><th>제목</th><td>" + (compact ? "<span class='p-table__subject_text'>소상공인 지원 공고</span>" : "소상공인 지원 공고")
                + "</td></tr><tr><td title='내용'>지원사업 안내</td></tr><tr><th scope='row'>파일</th><td>"
                + "<ul class='" + (compact ? "p-attach" : "view_attach") + "'>" + items + "</ul></td></tr></table>" + (compact ? "</div>" : "");
    }
    static String selectQuery(Case sample, String id) {
        return (selectCompact(sample) ? "" : sample.board().equals("25") ? "key=352&" : "bbsNo=17&") + "atchmnflNo=" + id;
    }
    static String selectItem(Case sample, String id, String name, boolean preview) {
        ...
        if (selectCompact(sample)) return "<li class='p-attach__item'><a class='p-attach__link' href='" + download
                ...
                + (preview ? "<a class='p-attach__preview' href='./previewBbsFile.do?atchmnflNo=" + id + "'>미리보기</a>" : "") + "</li>";
    ...
    @Test void registryHasThreeDistinctImmutableProfiles() {
        try (var context = new AnnotationConfigApplicationContext(StandardBbsAttachmentProfileConfiguration.class, AttachmentDiscoveryProfileRegistry.class)) {
            ...
            assertThat(registry.selectProfileList()).hasSize(3);
    ...
    @Test void allTwelveProfilesCoexistAndLegacyMimeDoesNotLeakToExistingProviders() {
