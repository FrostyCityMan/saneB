...
| 감사 로그 화면 | 예정 | Phase 6 API 조회 계약 보강 |
...
즉시 다음 개발 대상은 감사 로그 조회 화면이다. 이유는 공고, 검증, 매칭, 신청 진행의 운영 흐름이 늘어나면서 누가 어떤 상태를 변경했는지 확인할 수 있는 운영 추적성이 필요하기 때문이다.
