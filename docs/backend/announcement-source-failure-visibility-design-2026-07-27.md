...
- 단계: 구현, Backend Gate, 운영 배포 및 브라우저 QA 완료
- 자동 수집 Gate: `ASSCH-000001` 승인 상태로 재활성화
- 운영 수집 Gate: `ASRUN-000072`, 안정 출처 227곳, 실패 0곳
- 실패 사유 저장 Gate: 통과
- 관리자 조회 Gate: 운영 화면과 진단 모달 검증 완료
- 권한 Gate: 기존 `OPERATOR`, `APPROVER`, `ADMIN` 조회 권한을 유지한다.

현재 시스템은 URL별 수집 결과와 최근 오류를 DB에 저장하고 관리자 화면에서
진단 유형, 한글 원인, 권장 조치, 최근 실행 이력을 확인할 수 있다.

구현 전 기준인 2026-07-27 운영 실행 `ASRUN-000009`에서는 다음 차이가 확인됐다.

- 완전 실패: 안양시청 1곳
- 부분 누락: 과천시청, 성주군청 2곳에서 총 4개 행
- 실행 집계 실패·제외: 총 5건

최종 구현에서는 `TRANSPORT_FAILED`, `PARSER_FAILED`, `PARTIAL_FIELDS`,
`SEMANTIC_MISMATCH`, `IRRELEVANT_CONTENT`를 분리 집계한다. 운영 출처에서
반복 실패한 기관은 비활성화하고, 해당 원인과 재검증 조치를 관리자 화면에 남긴다.
...
기존 migration은 수정하지 않고 `V56`부터 `V60`까지 additive migration을 적용했다.

- `V56`: 출처 게시판 종류, 수집 정책, 의미 검증 정보, 정적 키워드 규칙,
  수집 항목 판정 상태와 제외 건수 추가
- `V57`: 밀양시·함양군 공식 공고 주소 보정
- `V58`: 운영자용 공식 바로가기와 실제 수집 endpoint 분리
- `V59`: 새올 고시공고 행 선택 범위 축소
- `V60`: 성북구·광산구·창원시 목록 행 선택 안정화

신규 의미 규칙 테이블은 `announcement_source_semantic_keyword_rules`이며,
출처 분류는 기존 `local_government_notice_sources`에 additive 컬럼으로 저장한다.
...
- [x] 정상, 부분 누락, 완전 실패 fixture를 각각 준비한다.
- [x] 실행 상세의 URL별 결과와 DB source result 행이 일치한다.
- [x] 출처별 최근 이력이 최신순으로 pagination된다.
- [x] 전송 실패, 파싱 실패, 필드 누락, 의미 불일치를 중복 없이 집계한다.
- [x] 오류 코드별 한글 제목과 권장 조치가 표시된다.
- [x] 정의되지 않은 오류 코드도 안전한 기본 문구로 표시된다.
- [x] `USER`, `PARTNER` 접근이 403으로 차단된다.
- [x] `OPERATOR`, `APPROVER`, `ADMIN` 조회 권한이 유지된다.
- [x] 재수집 요청이 승인 대기로 생성된다.
- [x] 360px, 768px, 1024px, 1440px에서 모달과 표를 확인한다.
- [x] 키보드로 상세 열기, `Escape` 닫기, 호출 버튼 초점 복귀가 가능하다.
- [x] 전체 테스트와 `bootJar`가 성공한다.
- [x] PostgreSQL/Flyway Backend Gate가 성공한다.
- [x] `SELECT *`, `${}`, `th:utext` 금지 패턴이 없다.
...
운영 DB의 전체 출처 244곳은 다음 구성이다.

- 기초지자체 227곳
- 시·도 15곳
- 행정시 2곳

클라이언트가 말한 226개는 일반적인 기초지자체 집계 기준이고, 현재 seed에는
2026-07-01 출범한 검단구를 포함한 기초지자체 227곳과 광역단체·행정시 17곳이
추가되어 총 244곳이다.

운영 API 기준 게시판 분류는 다음과 같다.

| 게시판 종류 | 출처 수 | 정책 |
|---|---:|---|
| 고시·공고 | 30 | 전체 수집 |
| 지원사업·모집공고 | 10 | 전체 수집 |
| 일반 공지사항 | 203 | 정적 키워드 필터 |
| 보도자료 | 1 | 수집 제외 |
| 미확인 | 0 | 활성 불가 |

수집 정책 집계는 `COLLECT_ALL` 40곳, `KEYWORD_FILTERED` 203곳,
`EXCLUDED` 1곳이다.

### 15.2 기존 데이터 보존

구현 전 기준은 실행 `ASRUN-000009`, 검수대기 293건이었다. 삭제 전 dry-run에서
유효 후보 144건, 제외 대상 149건으로 분류했다. 기존 유효 공고는 유지하고,
무관 자료는 삭제하지 않고 비활성·제외 상태로 이관했다. QA 실행으로 명확히
식별되지 않는 수집 이력, 실행 이력, 감사 로그는 삭제하지 않았다.

최종 운영 수집 이후 검수대기는 1,764건이다. 이 수치는 반복 운영 수집에서
추가된 유효 후보를 포함하며, 검수 화면의 기본 필터는
`REVIEW_PENDING + ACCEPTED`이다.

### 15.3 안정 출처 운영 Gate

최종 실행 `ASRUN-000072` 결과는 다음과 같다.
...
- 정적 규칙 제외: 1,210건
- 실패: 0건
- 진단: 정상 71곳, 무관 게시물 제외 156곳

자동수집 일정 `ASSCH-000001`은 `APPROVED`로 재활성화했다. 다음 실행 시각은
2026-07-28 12:30 KST이다.

### 15.4 반복 실패 출처

외부 기관 응답 문제를 세 번 이상 재현한 16곳은 전체 PASS에 포함시키지 않고
운영 수집을 비활성화했다.

| 원인 | 관리코드 |
|---|---|
| TLS 협상 실패 | `LGS-000002`, `LGS-000065`, `LGS-000094`, `LGS-000129`, `LGS-000132`, `LGS-000184`, `LGS-000192`, `LGS-000209` |
| HTTP 오류 | `LGS-000098` |
| 응답시간 초과 | `LGS-000008`, `LGS-000060`, `LGS-000061`, `LGS-000124` |
| 운영 DNS·endpoint 연결 불가 | `LGS-000229`, `LGS-000239` |
| HTTP 200 빈 목록과 timeout 반복 | `LGS-000180` |

보도자료 출처 `LGS-000084`는 오류가 아니라 정책상 `EXCLUDED`이다.
차단 출처는 관리자 화면에서 진단 사유와 권장 조치를 확인한 뒤 단일 출처
재검증을 거쳐서만 다시 활성화한다.

### 15.5 브라우저 QA

운영 화면 `/app/admin/announcement-sources`와
`/app/admin/collected-announcements`를 360px, 768px, 1024px, 1440px에서
검증했다.

- 문서 가로 넘침 없음
- 콘솔 오류와 요청 실패 없음
- 게시판 종류·수집 정책·진단 사유 필터 노출
- 검수대기 기본값은 유효 후보만 표시
- 내부 UUID 화면 노출 없음
- 순천시 진단 모달에서 최근 실패 원인과 권장 조치 확인
- `Escape` 닫기와 호출 버튼 초점 복귀 확인
