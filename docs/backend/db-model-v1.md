...
V56은 출처 게시판 유형을 `LEGAL_NOTICE`, `SUPPORT_RECRUITMENT`, `GENERAL_NOTICE`, `PRESS_RELEASE`, `UNVERIFIED`로, 수집 정책을 `COLLECT_ALL`, `KEYWORD_FILTERED`, `EXCLUDED`로 분리한다. 244개 출처는 기초지자체 227, 시·도 15, 행정시 2이며 정적 의미 분류 결과는 일반 공지 209, 고시·공고 24, 지원·모집 10, 보도자료 1이다. 보도자료 출처는 OFF로 전환하고, 일반 공지는 DB seed의 포함·제외 키워드에 따라 `ACCEPTED`, `EXCLUDED`, `REVIEW_REQUIRED`로 판정한다. 키워드는 설명 가능한 판정 근거일 뿐 점수·추천 확률로 사용하지 않는다.
