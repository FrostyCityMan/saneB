...
2026-09-15부터 새 catalog의 COMPLETE_TEXT 기대값은 역할 규칙·역할/사유·textHash/blocksHash/assessmentHash도 고정한다. 실제 파일 결과의 `roleAssessmentHash`와 textHash를 같은 고정 입력에 대조한다. 과거 역할 근거 없는 결과에 임의 지문을 추가하거나 새 역할 기대값에 과거 결과를 붙일 수 없다. 누락/null/자료형 변조·다른 파일 지문은 실패이며 원장 전체 hash를 다시 계산해도 이 결합을 우회할 수 없다. 역할 근거 자체·원문·파일명은 원장에 복제하지 않는다. UNKNOWN 기대 일치는 음성 시험 성공일 수 있지만 정상3공고를 채우지 않는다. `allTextComplete`는 기존 추출 품질 의미이며 관리자 최종 승인·정상 후보와 같지 않다.
