...
> 2026-09-25 긴 서명란 규칙 명시 선택: 기존 v2의 `segmentRuleVersion`/`analysisVersion` 허용 목록에 `segment-role-1.0.4`를 추가한다(현재0/2/3/4). 서버 지문은 `27dfa69f39bea3c47e1bf40b20bf01471f2432bc08ee143355e4e849089406fe`이다. 기본0·생략 수정의 현재 버전 유지·진단1 거부·버전별 GET 무생성·평가 FK 결합·QA stale 계약은 그대로다. 화면 선택은 초안 저장이며 게시/ENFORCE/기존 데이터 적용을 실행하지 않는다. v1/응답 wrapper/Flyway는 변경하지 않는다. 아래 허용 목록은 각 시점의 이력이다. 검증과 운영 적용 여부는 장기 진행 기록에서 별도로 확인한다.
