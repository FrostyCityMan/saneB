...
현재 지자체 SourceBinding11개는 이 운영 snapshot의 기관 code·목록 parser와 모두 일치한다. 나머지212개는 모델 결합이 없으며, 동일한 목록 parser를 쓴다는 이유로 첨부 지원 대상으로 자동 승인하지 않는다. 전체 Provider 증거 계획에서 이212개를 제외하거나 표본 수만으로 정상 처리율을 계산하지 않는다.
