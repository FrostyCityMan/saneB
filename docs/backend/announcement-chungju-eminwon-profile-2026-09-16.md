...
- [~] 같은3표본의 Linux 격리 추출/텍스트 역할/종합 판정 경로와 수동 workflow 입력을 연결했다. 두 음성은 요청0, 양성의 HWP 전체 파일만 처리하며 실제 품질/역할은 원격 결과로 판정한다. 기존 공통44요청/80MiB·파일20MiB 상한, 관리자 최종 검증·운영 쓰기0을 유지한다. 새 Node 판정기17건+기존 제천32건=49/49 통과다.
...
- [ ] 같은 SHA 전체 Linux/실제 추출·역할·worker DB/API/정책 QA 및 운영 검증은 후속 근거가 필요하다. 새 모델 추가만으로 정상 공고 수나 정책을 활성화하지 않는다.
