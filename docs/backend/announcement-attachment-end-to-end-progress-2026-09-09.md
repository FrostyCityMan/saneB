...
| 1 DB·API 계약 | [~] | 09-14 V81·3단계 projection·v2 대기열 필터의 QA 브랜치 실제 PG 검증 통과. 기존 v1·과거 migration 보존. P3 잔여 계약·전체 실제 catalog·운영 확인은 미완료 |
| 2 상시 worker·Provider | [~] | worker/scheduler·총12개 첨부 profile·고정 예약/ENFORCE binding·24시간 재확인/checkpoint 구현. QA 브랜치 실제 Linux worker8건·DB 계약 통과. 기존 BBS9사례14파일 발견/다운로드 기록과 구분하며 전체 profile/운영 연속 수집은 남음 |
...
| 6 자동·실파일 QA | [!] | 09-14 일곱 번째: 주 DB/runtime/worker와 독립215/215 통과·정리 성공. 부모2건은 QA_CHILD_PROCESS_LIMIT로 기동 실패. 자식128개 제한 유지·상위 JVM 병렬성 축소 후 재검증 필요. 직전 독립 대량 분할1건의 간헐 실패와 전체 Provider 실제 기대값/전수 검증도 잔여 |
