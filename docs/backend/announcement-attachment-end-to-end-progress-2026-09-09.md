...
- [x] Node6·Python신규4/기존25 통과, 예약 파일로 서울 중복 실행 차단도 확인했다. Java/추출기/운영 코드는 변경하지 않았다. 기존8요청·6,399,210byte에 최대6요청·23MiB를 예약하며 결과 확인 전 사용량을 초기화하지 않는다.
- [x] 8760141 Linux36035318540 completed/success와 artifact XML을 확인했다. root3047=2754통과/293조건부 생략,추출기149·패키지20·job209·migration18·정책 부모2·runtime5·worker12·Flyway3 실패/오류0이다. 새 코드/실파일/서울 운영 성공과 구분한다.
- [!] 78cbf7b GitHub36037865775 함안 단계는 실패했다. 본문2시도TIMEOUT 후 상세DETAIL_DISCOVERY/TRANSPORT_TIMEOUT, 파일0은 미발견 상태이지 첨부 없음이 아니다. 실제3요청/2MiB·원본정리true/운영쓰기0·JUnit1실패 영수증을 확보했다. 누적11/60요청·8,496,362byte를 유지하며 동일 GitHub 실행은 반복하지 않는다.
- [~] 전체 CI 후속 DB 회귀는 계속 진행 중이다. 전체9Gate=8부분/1차단 유지, 실제 구간 엔진 worker/DB/API·정상 기대값·전체 수집원/운영 E2E가 남는다. 로컬 성공과 GitHub timeout의 원인은 미확정이며 다음 서울 실행은 AWS 갱신 대기다. 운영 변경·브라우저 실행은 없다.

### 2026-09-25 함안 첫 서울 격리 관측 경로 구현

- [x] 직전 함안 본문/HWP signature 근거는 progress다. 장기 goal 스킬에 따라 고정41306 한 공고의 실제 Linux 추출·구간 근거 관측 모드를 추가했다. 다른 공고/지문·초과 예산·부분 품질의 정상 승격을 거부하며 기존 기본 모드와 운영 코드를 보존했다. [범위·성공/실패 기준·명령·남은 단계](announcement-haman-isolated-observation-2026-09-25.md)에 기록했다.
- [x] Java59·패키지20·Python25·Node5건 실패/오류/생략0, 준비1분53초 성공이다. 첫 합성 PARTIAL_TEXT 입력의 필수 사유 누락1건을 수정했고 검증 조건은 유지했다. 실제 이전 영수증을 읽는 누적8/60요청·6,399,210/100,663,296byte 사전 보호 조건도 통과했다. 본체/추출기1.0.11/bootJar는 동일 코드 결과 재사용이다.
- [!] AWS 인증 확인은 갱신 필요였다. 사용자에게 갱신을 요청했으며 서울 실행·업로드·추가 외부 공고 요청은0이다. 원격 실행 결과가 없으므로 함안 텍스트 추출·구간 검증 성공으로 집계하지 않는다. 소유CA/Node/Java/Python/PG잔여0이다.
- [~] 전체9Gate=8부분/1차단 유지. 관측 구간 metadata와 명시 구간 엔진 worker/DB/API 평가 결합은 별개다. 실제 추출·추가 정상 표본·전체 Provider 기대값·운영 정책/기존 데이터·동일SHA 배포/업무 E2E가 남는다. 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 함안 지원사업 첫 참조와 전체 파일 signature 검증

- [x] 직전 HWP 컨트롤 식별은 progress다. 장기 goal 스킬에 따라 전체Provider 참조 공백을 대조하고 함안41306의 공식 제목과 전체 첨부1개를 확인했다. 검색/단축URL만으로 등록하지 않고 실제 pinned transport 검증 후 참조를 추가했다.
- [x] 함안 전용 실제2시험이21초 빌드에서 통과했다. 본문AVAILABLE/시도1/redirect0, 전체HWP1개101,888byte 다운로드/signature·원본정리true다. 제목/URL/예산 표적105건, catalog 포함85건·패키지20건·probe/bootJar 준비2분10초도 통과했다. [설계·명령·실파일 지문·한도·시도별 실패](announcement-haman-support-reference-qa-2026-09-25.md)를 기록했다.
- [x] 기존 달성 JUnit 영수증을 덮어쓰지 않는 전용 task/보고서 경로를 추가했다. Node5건 통과, 현재37참조/13기관/기대값1/정상0이며 기존36참조/기대값은 그대로다. 함안1건은 REFERENCE_ONLY, 최소3건·전체형식·실제 Linux추출/구간/worker DB/API 완료가 아니다.
- [x] 함안 캠페인 누적8/60요청·6,399,210/100,663,296예약byte다. 초기짧은응답/목록timeout/대조요청도 남겼으며 응답 차이의 원인은 미확정이다. 같은외부요청을 반복하거나 원장을 초기화하지 않았다. 달성57/60·보은160/162·남구58/60의 기존 한도도 그대로다.
- [x] 확대 회귀3분33초 성공: 첨부 모듈1,992=통과1,968/조건부 생략24·추출기149·패키지20, 실패/오류0. 최초 예상catalog36/실제37 실패1건은 예상 건수만 수정하고 정책 스냅샷14건으로 먼저 재검증했다. Node5·diff 통과, 소유Node/Java/PG잔여0이다. probe/bootJar는 이번 소스로 앞서 생성한 결과를 재사용했다.
- [~] 원격 동기화/이번 SHA CI는 별도로 확인한다. 전체9Gate=8부분/1차단 유지, 등록19개 중 참조 없는6개와 함안추가2건·전체Provider 기대값·상시worker/DB/API·정확한 승인 범위 적용·같은SHA 배포/브라우저가 남는다. 운영 변경 없으며 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 HWP1.0.11 미지원 컨트롤 식별

- [x] 장기 goal 스킬에 따라 미지원 레코드를 배치 정보로 추정하지 않고 공개 tag/ID를 확인했다.76=도형/85=그림이며, 원문 없는 고정 컨트롤 종류 진단만 확장했다. DB/API/migration/추출 품질·분류 규칙은 보존했다.
- [x] 서울51022 전체PDF/HWP 관측32.302초·1/1 성공. 남은 컨트롤은 **쪽 번호2/도형1**, 그림 내용을 확인한 것은 아니다. 동일body/binary/text/locator·문자/블록수·최종판정 검증, 두 파일PARTIAL_TEXT·REVIEW_REQUIRED 유지다. [설계·명령·실행 지문·결과](announcement-hwp-control-kind-diagnostic-v1.0.11-2026-09-25.md)에 기록했다.
- [x] 준비1분6초 Java46/추출기149/패키지20, 확대3분25초 root1989=1966통과/23조건부 생략·실패/오류0. Node4/Python24·PowerShell 구문·diff 통과. 동일 추출기/패키지/probe/bootJar는 준비 결과를 재사용했다.
- [x] 누적57/60요청·44,134,566byte·잔여3회. 원격unit/원본/임시 자원·S3 자기 객체·로컬 재생성 ZIP을 정리하고 영수증을 보존했다. 운영DB미사용/쓰기0·설치JAR불변/healthUP·소유Node/Java/PG잔여0을 확인했다.
- [!] 같은 달성 단일 공고의 최대6요청 재실행은 잔여3회로 불가하다. 예산을 초기화하거나 파일 일부만 검사해 전체첨부 QA로 대체하지 않는다.
- [~] 전체9Gate=8부분/1차단 유지. 그림/쪽번호/PDF 구조와 전체Provider 기대값, 상시worker DB/API·운영정책/기존데이터·동일SHA 배포/업무E2E는 남는다. 운영 변경 없으며 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 HWP 헤더 구조 실측과1.0.10 표 오류 해소

- [x] 직전40바이트 지원/실파일 반증은 progress다. 장기 goal 스킬에 따라1.0.9에 원문 없는 종류·길이·확장 상태·잔여byte 진단을 추가하고 strict IPC 검증과 고정51022 단일 모드를 연결했다. 128형태·정수·집계/순서/중복·추가 필드 제한을 지키며 품질을 진단만으로 승격하지 않는다.
- [x] 서울1.0.9 진단34.261초·1/1 성공에서 표19개 모두48바이트/빈 설명 뒤0값2byte를 확인했다. 공개 읽기/쓰기 형식과 대조하고 이 고정 형태만 지원한1.0.10을 재검증했다. 셀 구조/순서 검증과 다른 비영값·미지원 확장 거부를 유지한다. [계약·증거·실행 지문](announcement-hwp-header-diagnostic-v1.0.9-2026-09-25.md)을 기록했다.
- [x] 서울1.0.10 비교37.29초·1/1 성공. **TABLE_CONTROL_HEADER19→0**, 동일binary/text/locator와8,993자/347블록 유지다. 미지원record2/control3와 PDF 부분 품질이 남아 정상후보0·REVIEW_REQUIRED/최종관리자확인을 유지한다. worker DB/API·관리자 DRAFT 성공으로 대체하지 않는다.
- [x] 최종 준비1분6초·표적45/추출기147/패키지20 통과, 확대3분27초·root1988=1965통과/23조건부 생략·실패/오류0이다. Node4/Python24·PowerShell 구문·diff 통과, 동일 추출기/패키지/bootJar는 앞선 결과 UP-TO-DATE다. 선행f6e56c8 Linux36027350033 success이며 새 코드 CI는 별도다.
- [x] 두 단일 실행10회/6,979,810예약byte, 최신 누적52/60·40,644,661byte·잔여8회다. 두 실행의 unit/원본/임시 자원·S3 자기 객체·재생성 ZIP을 정리하고 영수증을 보존했다. 운영DB 미사용/쓰기0·JAR 불변/health UP·소유 Node/Java/PG 잔여0이다.
- [~] 전체9Gate=8부분/1차단 유지. 다음은 미지원record/control·PDF와 전체 Provider 기대값, 상시worker/DB/API·운영 정책/기존 데이터/동일SHA 배포·업무E2E다. 운영은 변경하지 않았고 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 HWP1.0.8 공통40바이트 헤더 지원과 실파일 반증

- [x] 직전 달성 전체 파일 추출은 원인 범위를 확인한 progress다. 장기 goal 스킬에 따라 공개 읽기/쓰기 형식을 대조하고 누락된40바이트 공통 표 헤더를 지원했다. 셀 geometry/문단/순서·미지원 구조·부분 품질 규칙은 유지하며 runtime1.0.8로 구분했다. DB/API/migration/역할/구간 규칙은 변경하지 않았다.
- [x] 1차1분9초 및 확대3분32초 성공. root1986=1963통과/23조건부 생략,추출기142·패키지20·Node4·Python24건 실패/오류0. 추출기/bootJar는1차 검증/생성 결과를 확대 실행에서 재사용했다. 선행eae2248 Linux36025606676은 마지막 조회 in_progress다.
- [x] 서울55.463초·3/3 관측 성공. 동일 본문/파일/text/locator 지문과 품질/판정을 확인했으나 **달성51022의 표 헤더 오류19회는 그대로이며4파일 모두 PARTIAL_TEXT**다. 이번 수정이 달성 오류의 해결책이라는 가정은 반증됐다. [형식 근거·실행 결과·다음 진단](announcement-hwp-common-header-v1.0.8-2026-09-25.md)에 기록했으며 임의 길이 완화로 정상 승격하지 않았다.
- [x] 누적42/60요청·33,664,851예약byte,잔여18회/66,998,445byte다. 운영 DB 미사용·쓰기0·설치 JAR 불변·health UP, 원격 unit/임시 자원 정리를 확인했다. 이전 요청량을 초기화하지 않는다.
- [~] 전체9Gate=8부분/1차단 유지. 다음은 고정 단일 표본의 실제 헤더 확장 구조·미지원 control 진단과 PDF 원인 식별이다. 현재 전체3건69MiB 상한은 잔여byte보다 커서 동일 실행을 반복하지 않는다. 전체 Provider 기대값·worker/DB/API·운영 정책/기존 데이터/동일SHA 배포·업무 E2E는 남으며 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 달성군3공고·전체4파일 서울 격리 추출

- [x] 장기 goal 스킬에 따라 앞선 다운로드 성공을 완전 추출로 간주하지 않고 고정 DALSEONG_OBSERVATION 모드를 추가했다. 본문/전체 파일/고정 binary 지문·추출 버전·예산·부분 품질의 검수 유지를 검사하며 기본 모드·운영 코드/DB/API/정책은 변경하지 않았다.
- [x] 서울63초·관측3/3 성공. 본문3건과 PDF1/HWP3 전부를 확보·추출했고 로컬 선행 파일 지문과 일치했다. **4개 모두 PARTIAL_TEXT, 3공고 모두 REVIEW_REQUIRED**, 정상 후보0·wholeText=false·관리자 최종 확인 요구 유지다. HWP 미지원 record/control와 표 헤더 원인을 구분했고 PDF 세부 원인은 미확정이다. [실행 지문·결과·예산·정리](announcement-dalseong-support-qa-2026-09-25.md)를 기록했다.
- [x] 신규13요청/8,075,889예약byte, 누적29/60·25,588,962/100,663,296byte다. 운영 DB 미사용·쓰기0, 운영 JAR 불변·health UP을 확인했다. 원격 unit/원본/임시 자원·S3 자기 객체·로컬 재생성 ZIP을 정리하고 영수증을 보존했다.
- [x] Java54·패키지20·Node4·Python24 실패/오류/생략0, 준비 빌드1분44초 성공. probe JAR 생성, 본체/bootJar는 변경 없는 UP-TO-DATE다. 최초 전송 전 로컬 표시명 검증 실패는 외부 실행 없이 수정 후 같은 plan으로1회만 제출했다. 선행3f4e6d7 Linux36023716864는 마지막 조회 in_progress로 이번 증분의 CI 통과를 주장하지 않는다.
- [~] 전체9Gate=8부분/1차단 유지. 다음은 PDF/HWP 부분 추출 개선·구간 근거·실제 worker/DB/API와 전체 Provider 기대값이다. 운영 정책/기존 데이터/동일SHA 배포·업무 E2E는 여전히 남는다. 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 달성군 지원사업3건의 본문·전체 파일 참조 확보

- [x] 이전 보은1.0.4 실파일 저장 검증은 progress다. 장기 goal 스킬에 따라 다른 미검증 프로필로 범위를 확장했다. 부산 본청 신규 표본의 HTTP401/구조 부재는 성공이나 정상 참조로 바꾸지 않고 중단했다.
- [x] 달성군51022/52145/51075의 공식 제목과 전체 파일 목록을 확인했다. 기존 pinned transport의3시험에서 PDF1/HWP3 다운로드/signature·임시 원본 삭제, 본문3시험에서 AVAILABLE/attempt1/redirect0을 검증했다.6/6 통과·실패/생략0이며 외부 binary를 Windows에서 추출하지 않았다.
- [x] 참조36/12기관/기대값1/정상0으로 확장, 기존33건과 태백 기대값을 보존했다.7엔진/19프로필 중 참조 없는 등록 프로필은7개다. 기존 API/migration/운영 정책/데이터/배포 변경은 없다. [달성 실행 지문·한도·남은 업무](announcement-dalseong-support-qa-2026-09-25.md)를 기록했다.
- [x] 표적 Java113·패키지20·Node4 통과, catalog 포함 bootJar 생성 성공. 확대 첨부 모듈1983=1960통과/23조건부 생략·실패0,3분20초 성공이다. 후속 예산 제한 증분과 Linux/운영 증거는 별도로 구분한다.
- [x] 최종 예산 제한 증분 계약22건1분39초 통과·실패/생략0, 소유 Node/Java/PG 잔여0을 확인했다. 선행 bd68cfa Linux36021816332 success이며 달성 변경은 후속 CI로 별도 검증한다. 원본 파일만 임시 삭제했고 비식별 영수증과 기존 사용자 미추적 파일은 보존했다.
- [~] 전체9Gate=8부분/1차단 유지. 다음은 달성 전체4파일의 서울 격리 추출·역할/구간 검토·worker/DB/API, 미확정 서문/서식/PDF와 전체 Provider 기대값이다. 현재 원장16/60요청·17,513,073예약byte를 보존한다. 운영 게시/ENFORCE/기존 데이터/동일SHA 배포·업무 E2E는 여전히 남는다. 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 보은221497의1.0.4 실제 저장·조회 검증

- [x] 직전 명시1.0.4 연결은 progress다. 장기 goal 스킬에 따라 메모리 후보 성공을 실제 저장 성공으로 대신하지 않고, 기존 누적 원장을 보존하는 단일 공고 모드(최대5요청/24MiB)를 추가했다. 운영 코드/DB/API/정책/기본값은 이번 변경하지 않았다.
- [x] Java29·패키지20·Node9·Python23 실패/생략0, PowerShell 구문0오류·diff 검사 통과. 최종 준비 빌드32초 성공이며 변경 없는 본체/bootJar/추출기는 UP-TO-DATE다.
- [x] 서울 고정1공고43.290초·1/1 성공. 동일 HWPX/텍스트와 이전 후보4 지문이 실제 worker4 저장·평가 FK·명시 GET·판정 결합 GET에서 일치했다.0/2/3 GET은 무생성이며 UNKNOWN1/NOTICE1/FORM2·검수/원문 확인/최종 관리자 확인 요구를 유지했다. [실행 지문·예산·정리](announcement-segment-seoul-qa-2026-09-24.md)를 기록했다.
- [x] 실제4요청/2,421,620예약byte, 누적160/162·108,895,847byte·잔여2회다. 원격 unit/원본/임시 DB/lease·S3 자기 객체·재생성 가능한 로컬 ZIP을 정리하고 영수증을 보존했다. 운영 JAR 불변·health UP·운영 쓰기0, 로컬 소유 Node/Java/PG 잔여0이다.
- [x] 선행 eaeb482 Linux36017762988 success에 이어 df74ec2 [Linux36019625775](https://github.com/FrostyCityMan/saneB/actions/runs/36019625775) success를 확인했다. 다운로드 XML root3031=2742통과/289조건부 생략, 별도 extractor133·패키지20·jobPG209·migration18·정책부모PG2·runtime5·workerPG12·Flyway3은 실패/오류/생략0이다. 이 CI는 긴 서명란 운영 코드 연결 기준선이며 이번 단일 probe 변경의 새 CI와 구분한다.
- [~] 전체9Gate=8부분/1차단·goal active 유지. 다음은 최신 Linux DB 결과, 미확정 서문/서식/PDF·참조 없는8프로필과 전체 Provider 기대값, 운영 정책/기존 데이터/동일SHA 배포·업무 E2E다. 남은2요청으로 최대5요청 worker를 다시 실행하지 않는다. 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 긴 서명란1.0.4 명시 정책·worker·조회 연결

- [x] 기준선 `eaeb482`에서 장기 goal·프론트엔드 기본/UI·UX/구현 스킬을 적용했다. 명시4 선택만 엔진·정책·DTO·구간 GET·화면 지문 계약에 추가하고 기본0·기존0/2/3·진단1 제외·v1·V1~V85는 유지한다. 기존 선택 상자와 변경 동의/QA stale/입력 보존을 재사용하며 라이브러리·화면 구조는 바꾸지 않았다.
- [x] SG-002의4 표본에187자 신청인/서명란을 넣어 NOTICE/FORM·서식 A/B 참고 전용을 검증했다. 동일 입력의3은 NOTICE/UNKNOWN이다. 실제 임시 PG의4 저장/버전별 GET/평가 FK·구버전 SHADOW 공존/확인 후 멱등 DRAFT/초안 QA stale 시험3개도 추가했다. 테스트 정의와 실제 PG 실행 결과는 구분한다.
- [x] 표적 Java42초, 확대 회귀/패키지/bootJar3분35초 성공. 확대 root2007=1985통과/22조건부 생략, 패키지20통과·실패/오류0. Node 화면58·보고서/실파일 실행기18통과, 두 변경 JS 구문 검사 통과다. 소유 단발 Node/Java/PG 잔여0이며 기존 사용자 `output/`, `scripts/qa/__pycache__/`는 보존한다.
- [~] 선행 `eaeb482` Linux36017762988은 마지막 확인 시 실행 중이다. 새 연결 코드의 Linux 실제 PG 및 공식 파일 worker4 저장은 아직 미검증이다. Windows initdb 기확인 차단을 반복 시도하지 않고 격리 Linux 경로를 사용한다. 기본 Git 줄바꿈 설정의 diff 검사를 적용하며 운영 DB/정책/worker 설정/배포는 변경하지 않았다.
- [~] 전체9Gate=8부분/1차단 유지. 외부 요청 추가0·보은누적156/162/잔여6회 유지. 다음은 이번 SHA Linux DB 결과와 고정221497 단일 공고의 worker4 저장 검증, 미확정 서문/서식/PDF·전체 Provider 기대값, 이후 승인된 운영 정책/기존 데이터/동일SHA 배포·업무 E2E다. 브라우저는 현재 명시 요청 정책상 미실행이다.

### 2026-09-25 긴 서명란 후보1.0.4 실파일 개선

- [x] 직전 보은1.0.3 실제 저장은 progress다. 장기 goal 스킬에 따라 남은 구간 원인을 고정 HWPX XML과 대조했다. 동의서의 신청인/복수 서명란187자를 확인하고 기존 전체 줄200자 한도 안에서만 후보1.0.4 필드 판정을 추가했다. 이전0~3/hash/기본값·worker/정책/DB/API·구간 경계는 보존한다.
- [x] 첫 표적/bootJar38초,확대 회귀3분36초 성공. root1999=1977통과/22조건부 생략·패키지20·Node8·Python22 실패/오류0, PowerShell 구문0오류·diff 검사 통과다. 선행 c477c65 Linux36014177794 success/다운로드 XML root3016 및 별도 suite 실패0을 확인했으며 새 후보 코드와 구분한다.
- [x] 서울 같은3공고73.631초·3/3 성공. 실제 저장은1.0.3을 유지하고 메모리 후보의 원문/전체 범위/경계 불변을 검증했다.221497 UNKNOWN2→1로 동의서1구간 개선,221499의3개·PDF의1개 UNKNOWN은 유지한다. 정상 후보0·관리자 최종 검증 유지이며 새 버전 저장/정책 적용 성공은 아니다. [실측 지문과 잔여](announcement-segment-seoul-qa-2026-09-24.md)를 기록했다.
- [x] 공개 XML4회와 서버12회를 더해 누적156/162요청·106,474,227예약byte,잔여6회다. 원격 unit/원본/임시 DB/lease·S3 객체·로컬 자기 ZIP을 정리하고 영수증을 보존했다. 운영 JAR 불변·health UP·운영 쓰기0이다.
- [~] 전체9Gate=8부분/1차단·goal active 유지. 다음은 검증된 후보의 명시 실행 연결, 미확정 서문/서식/PDF·전체 Provider 기대값, 운영 정책/기존 데이터/동일SHA 배포·업무 E2E다. 현재 브라우저는 사용자 명시 요청 정책상 미실행이다.

### 2026-09-24 격리 QA 일괄 승인 후 보은1.0.3 실제 저장 검증

- [x] 최신 모든 격리 QA 요청 승인을 앞서 요청한 보은 추가30회에 적용했다. 기존128회 사용량을 유지하고 누적162회/240MiB로 관리한다. 장기 goal·현황 감사 스킬로 중복 진단과 미실행 저장 검증을 구분한 뒤 기존 BOEUN_STRUCTURAL 경로를 로컬 전송 실행기에 연결했다. 운영 코드/DB/API/정책은 수정하지 않았다.
- [x] 서울 고정3공고75.058초·3/3·실패/생략/중단0. 명시 구간1.0.3의 실제 worker 저장·평가 입력 FK·버전 GET·판정 결합 GET·구버전 조회 무생성을 검증했다. 두 HWPX와 PDF의 전체 텍스트/분석 지문은 고정값과 같으며 UNKNOWN3/2/1·검수/최종 관리자 확인 요구를 유지했다. [실행·판정·예산·정리 근거](announcement-segment-seoul-qa-2026-09-24.md)를 기록했다.
- [x] 신규12요청/7,391,673예약byte, 누적140/162회·94,888,250예약byte다. 원격 unit/원본/임시 DB/lease/전송, S3 자기 객체, 로컬 ZIP을 정리했고 영수증을 보존했다. 운영 JAR 불변·health UP·운영 쓰기0이다.
- [x] 로컬 Java26·Node8·Python22건 실패/생략0,22초 준비 빌드 성공. 변경 없는 패키지20·bootJar는 UP-TO-DATE다.614850d Linux36012304604 success/다운로드 XML root3013=2727통과/286조건부 생략·별도133/20/206/18/2/5/12/3 실패·오류0을 확인했다. c477c65 CI는 마지막 조회 실행 중이다.
- [~] 전체9Gate=8부분/1차단·goal active 유지. 이번 성공은 실제 새 버전 저장 경로의 진척이며 정상 후보·검수량 감소·Provider 전체 성공이 아니다. 운영 정책/기존 데이터/배포 및 현재 명시 요청 정책상 브라우저는 실행하지 않았다.

### 2026-09-24 남구44381 구간 미확정 원인 진단

- [x] 직전614850d는 동일 HWP의 정상 추출1건 개선이라는 progress다. AGENTS/Git/구간 설계·구현을 확인하고 장기 goal 스킬에 따라 판정 완화 전에 실제 구조 근거를 추가했다. 이번 변경은 QA 전용이며 운영 분류/추출기/DB/API/정책에는 변경이 없다.
- [x] 남구44381 단일 모드에 파일/text hash·수량1·5요청/24MiB를 고정하고 원문 없는 사전/위치/형태 숫자 진단을 검증했다. 선행 누적54요청을 실시간 차감한 서울1회27.817초·1/1 성공이다. 동일 body/binary/text/segment hash, COMPLETE_TEXT·구간7UNKNOWN·검수/최종 관리자 확인 유지를 확인했다.
- [x]160개 줄에서 신청기간/방법·제출서류·서식 용어는 있지만 현재 사전의 대상/지원내용/신청인/서명 표지는 없다. 이는 내용 부재의 증거가 아니며, 제출서류의 서식 참조를 실제 서식 경계로 오인했을 가능성도 미확정이다. [정확한 관측/해석 경계/다음 행동](announcement-namgu-three-stage-qa-2026-09-24.md)을 기록했다. 근거 없는 정규식 완화나 정상 승격을 하지 않는다.
- [x] 실제4요청/2,180,578byte, 누적58요청/28,590,407byte·잔여2요청이다. 원격 unit/원본/임시 DB/전송·S3 자기 객체·로컬 ZIP을 정리하고 영수증을 보존했다. 운영 JAR 불변·health UP 및 운영 쓰기0을 확인했다.
- [x] 표적64=62통과/조건부 생략2,패키지20·Node4·Python22건 실패/오류0. Python 최초 들여쓰기1오류는 수정 후 재검증했다. PowerShell 구문0오류·diff 검사 통과, bootJar/추출기는 변경 없는 산출물 UP-TO-DATE다. 소유 단발 Node와 로컬 Java/PG는 종료했다.
- [~] 전체9Gate=8부분/1차단·goal active다. 전체 Provider 정답과 실제 worker DB/API·운영 정책/기존 데이터·동일SHA 배포/브라우저 E2E는 남는다. 브라우저는 현재 명시 요청 정책상 미실행이다.614850d Linux36012304604는 실행 중으로 확인했으며 이번 QA 변경의 전체 CI 통과로 대신하지 않는다.

### 2026-09-24 HWP1.0.7 닫힌 하이퍼링크 표시 텍스트 지원

- [x] 장기 goal 스킬에 따라 공개 HWP 규격·필드 작성 구현과 현재 코드를 대조하고, 같은 문단 안의 정상 `%hlk` 표시값을 지원했다. URL/명령은 실행·다운로드·출력하지 않는다. 기존 표시 텍스트 순서와 필드별 분리 scope를 보존한다. 미지원 필드/손상/중첩/문단 간 범위/알 수 없는 record는 부분 처리를 유지한다. inline 종료에 가상의 CTRL_HEADER를 요구하던 진단을 바로잡았다.
- [x] runtime/extractor1.0.7·고정 사유31종으로 구분했다. DB/Flyway/API/A·B/FORM/구간 정책/자동 활성화 금지는 불변이다. 이전 runtime/정책 QA를 새 버전에 재사용하지 않는다.
- [x] 1차 표적/패키지/bootJar 검증1분3초 성공 후 확대 회귀3분15초 성공. root1966=1944통과/22조건부 생략·실패/오류0, 추출기133건 실패/생략0. 패키지20건·bootJar는1차 실행/생성한 동일 코드 결과 UP-TO-DATE다. Node4·Python21·PowerShell 구문0오류·diff 검사 통과다.
- [x] 서울 고정3건45.706초·관측3/3 성공.44381은 동일한 body/binary/text hash와3,307자/160블록을 유지한 COMPLETE_TEXT로 개선됐다. 전체 첨부 텍스트 처리는 완료했으나 구간7개 문맥 미확정으로 REVIEW_REQUIRED/ATTACHMENT_CONTEXT_REVIEW 및 최종 관리자 검증을 유지한다.44466/42871은 부분 처리 그대로다. [실파일 결과·지문·해석 경계](announcement-namgu-three-stage-qa-2026-09-24.md)를 기록했다.
- [x] 선행3회 영수증을 차감한 새 실제12요청/6,540,257byte, 누적54요청/26,409,829byte다. 잔여6요청으로15요청 상한의3건 재실행은 불가하며 예산을 초기화하지 않는다. 원격 unit/원본/임시 DB/transport·S3 자기 객체·로컬 전송 ZIP을 정리했다. 운영 JAR 불변·health UP을 확인했고 운영 DB/정책/worker/배포는 변경하지 않았다.
- [x] 선행48fb835 Linux36007924157 success 상태를 확인했다(이번 artifact 미조회).225dae5 Linux36009981975는 실행 중이며 새1.0.7 전체 CI 통과 근거가 아니다.
- [~] 전체9Gate=8부분/1차단·goal active다. 정상 파일 추출과 정상 후보/관리자 승인/상시 worker 저장은 구분한다. 전체 Provider 기대값·실파일 DB/API·정책 게시/기존 데이터·동일SHA 운영 배포/브라우저 E2E는 남는다. 현재 브라우저 명시 요청 정책상 미실행이다.

### 2026-09-24 HWP1.0.6 부분 처리 사유 진단과 서울 실파일 확인

- [x] 직전48fb835은 실제 구조 metadata로 남구 원인 범위를 좁힌 progress다. AGENTS/현재 Git/진행 기록을 확인하고 장기 goal 스킬에 따라 미확정 원인 식별→고정 코드 계약→단위/실파일 검증을 수행했다. 구간 분류/A·B 정책·텍스트 순서/정상 판정 기준은 변경하지 않았다.
- [x] 추출기와 runtime identity를1.0.6으로 구분하고 IPC에 원문 없는 부분 처리 사유29종/count를 추가했다. 표/셀/문단/inline/컨트롤 원인을 분리하고 고정 코드·수치·순서·품질 모순을 검증한다. 구버전 IPC 생략은 호환하고 새 HWP 누락은 실패한다. 기존 runtime/정책 QA 지문을 재사용하지 않으며 운영 설치는 바꾸지 않는다.
- [x] 서울 고정3건47.050초·3/3·실패/생략/중단0이다. 본문/binary/text는 이전과 동일하고 PARTIAL_TEXT/검수를 유지한다.44381은 미지원 inline1·MISSING_CONTROL1·미지원 control1로 범위를 좁혔으며 표/셀 실패는 관측되지 않았다. MISSING_CONTROL은 inline placeholder의 후속 이벤트일 수 있어 원본 앵커 유실로 단정하지 않는다. [원인·해석 경계와 지문](announcement-namgu-three-stage-qa-2026-09-24.md)을 기록했다.
- [x] 기존30요청/13,329,315byte 차감 후 최대15요청/72MiB로 실행했다. 실제12요청/6,540,257byte를 더해 고정3건 누적42요청/19,869,572byte다. 원격 unit/원본/임시 DB·S3 자기 객체·로컬 ZIP 정리 및 운영 JAR 불변/health UP을 확인했다. 비식별 영수증은 보존했다.
- [x] 추출기123·표적43·패키지20·Node4·Python21 실패/생략0. probe 변수명 충돌 컴파일1건 수정 후55초 성공이다. 확대 회귀3분16초/root1965=1943통과/22조건부 생략·실패/오류0이다. 선행047a1d8 Linux36006726822의 artifact root3007/286생략 및 별도122/20/206/18/2/5/12/3 실패·오류0을 확인했다. 새1.0.6 CI 근거로 대신하지 않는다.
- [~] 다음은 실제 컨트롤 종류·범위·텍스트 보존 조건을 확인하고 지원 가능한 구조를 구현하는 것이다. 이번 변경은 진단 기능이며 남구 부분 추출 해소/정상 후보 달성이 아니다.7엔진/19프로필/catalog33참조/11기관/기대값1/정상0·전체9Gate=8부분/1차단·goal active를 유지한다. 전체 Provider·상시 운영/정책·최종 확인/DRAFT·기존 데이터·동일SHA 배포/E2E는 남는다. 운영 데이터/정책/worker/배포와 현재 정책상 브라우저는 실행하지 않았다.

### 2026-09-24 남구 HWP 구조 진단 연결과 실제 원인 범위 축소

- [x] 직전047a1d8은 남구3건 실제 관측/회귀/커밋·푸시를 마친 progress다. 현재 HEAD와 worktree를 확인하고 `long-goal-operating-protocol`에 따라 실패 이유를 관측할 수 있는 검증 경로부터 연결했다. 추출기의 검증된 숫자 hwpStructure를 공통 관측 보고서에 추가하고 부분 문서의 역할/구간 성공 금지·잘못된 진단 거부를 시험했다.
- [x] 서울 재관측3/3·44.502초·실패/생략/중단0, 본문/binary/text 지문은 선행과 모두 동일하다.44466의 tag87 1개,42871의 tag76 12개/tag82 8개는 현재 코드의 부분 처리 직접 조건이다.44381은 알려진 종류만으로 원인을 구별할 수 없어 표/컨트롤/앵커별 상세 원인이 미확정이다. 부분 추출 해소나 정상3건으로 계산하지 않는다. [실측/해석 경계](announcement-namgu-three-stage-qa-2026-09-24.md)를 기록했다.
- [x] 이전 실시간 영수증18요청/6,789,058byte를 차감하고 새 상한15요청/72MiB로 실행했다. 실제12요청/6,540,257byte를 더해 고정3건 누적30요청/13,329,315byte다. 운영 JAR/health 불변, 원본·unit·임시 DB·S3 자기 객체·로컬 ZIP 정리를 확인했다. 원본/전송 자료는 제거하고 비식별 영수증을 보존했다.
- [x] 표적44·패키지20은34초 성공/실패·생략0. 확대 관측/HWP 시험은1분28초,root68=66통과/조건부 생략2·실패/오류0이다. 변경 없는 extractor122·패키지20·bootJar의 UP-TO-DATE는 재실행과 구분했다. Node4·Python21건 실패/생략0, PowerShell 구문0오류다. 선행 Linux36006726822는 실행 중이며 중복 실행하지 않았다.
- [~]7엔진/19프로필/catalog33참조/11기관/보관 기대값1/정상0·전체9Gate=8부분/1차단·goal active 유지. 다음은 부분 처리 사유의 고정 코드·수치 진단으로44381의 표/컨트롤 연결 원인을 특정하고 검증된 구조만 지원하는 것이다. 전체 Provider·운영 정책/기존 데이터·최종 검수/DRAFT·동일SHA 배포/E2E는 여전히 미완료다. 운영 코드/DB/API/정책/worker/배포를 바꾸지 않았고 브라우저는 현재 명시 요청 정책상 미실행이다.
...
- [x] BOEUN_SEGMENT의 임시 정책/snapshot만 명시1.0.2에 고정했다. 기존3공고 분모와 일반 BOEUN 파일 엔진은 보존했다. 이전 관측 고정 hash를 실제 저장 분석/평가 입력 FK/GET과 대조하고, 기본1.0.0 GET의 NOT_ANALYZED·무생성·무대체도 검사한다.
- [x] 표적 Java24·패키지20은 실패/오류/생략0이며31초 성공이다. `attachmentOfficialWorkerProbeJar`를 재생성했고 production 코드/bootJar는 e3ead92와 동일하여 UP-TO-DATE다. Python18·Node7·PowerShell 구문0오류·git diff --check도 통과했다. Python은 -B로 실행했으며 기존 캐시 삭제 차단을 우회하지 않았다.
- [x] 서울 고정3공고 실파일 QA는3/3·70.279초·실패/생략/중단0이다. e3ead92 production codeHash와 변경된 probeHash를 묶어 실행했다. SSM87d32cd1-0a36-4ea1-bb8e-3f22290187e7 및 [근거 기록](announcement-segment-seoul-qa-2026-09-24.md)을 확인했다. 두 HWPX의 NOTICE1개씩과 PDF UNKNOWN은 이전 후보 지문 그대로 실제 DB에 저장됐다.
- [x] 조회/최종 검수 컨텍스트·평가 결합은 일치하고 원문 확인/CONTEXT_ONLY·확정/link0을 유지했다. 운영 JAR 불변·health UP·unit/원본/lease 정리, S3 자기 객체 부재와 로컬 전송 ZIP 제거를 확인했다. 원본과 임시 실행 자료는 제거했으며 비식별 plan/result JSON은 보존했다.
- [x] 선행7회 영수증+수동 구조 확인 상한100요청/68,518,927byte를 재대조하고 이번 상한30요청/72MiB로 실행했다. 실제 예약12요청/7,391,673byte를 합친 누적112요청/75,910,600byte, 잔여20요청이다. 추가 공개 요청을 무제한 승인으로 해석하지 않는다.
- [~] 세 공고 모두 REVIEW_REQUIRED, UNKNOWN5/4/1·원문 수동 확인 요구 유지다. 실제 저장 연결은 진척이나 정상 후보/사람의 검수 시간 감소는 달성하지 못했다. 전체 Provider 기대값·UNKNOWN/부분 추출 해소·관리자 새 버전 선택/표시·최종 확인/DRAFT·운영/브라우저 E2E는 남는다.
- [~] e3ead92 Linux35992333497의 신규 PG2건을 포함한 전체 결과는 현재 실행 중이며 이 실파일3건과 범위를 구분한다. 운영 DB/정책/ENFORCE/기존 데이터/배포는 변경하지 않았다. 브라우저는 현재 요청 정책상 미실행이다. Gate8부분/1차단·ATT62·SEG10과 goal active를 유지한다.
...
- [x] 서울 고정3공고 위치 진단은3/3·72.753초·실패/생략0이다. 두 HWPX의 대상/지원/기간은 모두0번 구간에서 새 문법에 일치했고,1번 안내 구간에는 뒤쪽 기간만 있었다. [실행 근거](announcement-segment-seoul-qa-2026-09-24.md)에 위치/영수증/정리를 기록했다.
- [x] 알려진 binary hash의 공개 XML에서 주표제 끝 `모집 공고(3분기)`를 확인했다. 정확한 분기 표제를 별도1.0.2 경계/역할 규칙에 추가했다.1.0.0/1.0.1 재현성·서문 UNKNOWN·내부 안내/FORM 경계·전체 범위·부분 추출/PDF 검수는 보존한다. 다른 구간 근거를 합치지 않는다.
- [x] Java160·패키지20·Python18·Node 보고서10/launcher7·PowerShell 구문 검사 통과다. 중간160건 중1실패는 감축한24MiB와 임시 정책의 이전32MiB 불일치로 확인하고 fixture까지 정정했다. 최종38초 성공, bootJar는 앞선1분2초 실행에서 만든 동일 production 코드 산출물 UP-TO-DATE다.
- [x] 선행3143eff [Linux35988154022](https://github.com/FrostyCityMan/saneB/actions/runs/35988154022) success 및 XML 확인: root2913=2636통과/277생략/실패·오류0. 별도 extractor122·패키지20·jobPG198·migration18·정책부모PG2·runtime5·workerPG12·Flyway3은 실패/오류/생략0이다. `build/qa-results/linux-35988154022`에 보존했으며1.0.2의 전체 회귀 성공 근거로 쓰지 않는다.
- [x] 새1.0.2 서울 실파일 대조는3/3·71.630초·실패/생략0이다. 보은 HWPX2개 각각에서 NOTICE1개·필수 근거4개를 확보했다. 기존/후보 구간 수7→8,5→6이며 앞쪽 서문은 보존했다. PDF는 UNKNOWN 유지다. 상세 영수증은 위 실행 근거 문서의 SSM946fe091-c139-43df-8d73-2083e592d37f를 따른다.
- [~] UNKNOWN 개수5/4/1·REVIEW_REQUIRED는 유지한다. 역할이 확인된 공고 범위가 늘어난 진척이지 정상 후보 증가/실제 검수 시간 감소/최종 확정의 증거는 아니다.1.0.0 worker DB/API와 후보1.0.2 메모리 분석은 별개이며 후보 저장/정책 적용은 없다. 전체9Gate·ATT62·SEG10과 goal active를 유지한다.
- [x] 누적100요청/68,518,927예약byte로 승인132요청/240MiB 이내다. 다음 같은 고정 표본 QA에는 잔여32요청을 넘는 상한을 예약할 수 없다. 운영 JAR/DB/정책 불변·임시 unit/원본/lease 정리를 확인했다. 현재 요청 정책상 브라우저는 미실행이다.
- [x] 두 실행의 S3 소유 전송 객체 삭제/부재·plan.cleaned=true 이후 검증한 로컬 자기 패키지2개를 제거했다. 코드에서 재생성 가능하며 운영 데이터 삭제는 없다. JSON/CI XML/사용자 output은 보존했고 Java/PostgreSQL/해당 Node QA 잔여0이다.
- [!] 이번 시험이 생성한 `scripts/qa/__pycache__/test_temporary_bbs_observation.cpython-312.pyc`의 정리는 도구 실행 정책이 명령을 거부했다. 우회 삭제하지 않고 미추적 상태로 남기며 커밋에서 제외한다. 실행 프로세스나 원문 파일이 아닌 재생성 가능한 Python bytecode 캐시다.
...
- [x] Java85·Node/Bash7·Python18·패키지20 실패/생략0. 표적29초, 패키지/bootJar22초 성공이다. Python alias 부재는 번들 실행기로 해결했으며 테스트를 생략하지 않았다.
...
- [x] 수정분 `bfa7c2d5654726c13fde3e6bb5824fd77f78d24d`를 QA 브랜치에 커밋·푸시하고 원격 SHA 일치를 확인했다. Linux35983635675는 선행 작업 종료 대기다. 중복 대기 실행을 만들지 않기 위해 이 후속 결과 기록은 다음 코드 증분까지 로컬에 보존한다.
- [!] 추가 로컬 `:test --no-daemon --max-workers=1`은 3분55초에 실패했다. 이번 실행 XML2899건=2612통과/11실패/276생략/오류0이며, 콘솔의 컨테이너 제외 집계는2877건/11실패/254생략이다. 실패11건 모두 initdb.exe CreateProcess error4551 실행 정책 차단으로 확인했다. 신규 실제 PG2건 통과를 뜻하지 않으며 Linux 결과와 분리한다. Node 보고서 판정기10/10은 통과했고 소유 Java/PostgreSQL·일회성 Node는 종료했다.
- [x] 후속 재조회에서 선행 `f303dc6` Linux35982286016 success 및 artifact XML을 확인했다. root2890=2615통과/275생략/실패·오류0, 별도 extractor122·패키지20·jobPG196·migration18·정책 부모PG2·runtime5·workerPG12·Flyway3은 실패·오류·생략0이다. BOEUN_SEGMENT를 포함한 실제 DB 준비5건도 통과했다. `build/qa-results/linux-35982286016`에 근거를 보존했다. 최신 검수 수정 `bfa7c2d` Linux35983635675/job107583988851은 이어서 실행 중이며 이 선행 성공을 새 PG2건의 근거로 사용하지 않는다.
...
- [~] 전체 추출4파일에도 UNKNOWN 구간이 남아 양성5공고 모두 검수 상태다. 수집 성공을 정상 후보·기대값 승인·정책 QA·검수량 감소 입증으로 바꾸지 않는다. 이번 경로는 최신 worker 업무 DB/API E2E가 아니다.
- [x] 두 실행 모두 운영 JAR 불변·health UP·임시 unit 종료·원본 정리·전송 파일 정리를 확인했다. S3 자기 객체와 로컬 패키지2개를 정리했고 JSON 근거는 보존했다. 운영 DB/정책/worker/기존 데이터는 변경하지 않았다.
- [x] `6900e3c`의 [Linux35975514461](https://github.com/FrostyCityMan/saneB/actions/runs/35975514461) success 및 XML 확인: root2875=2600통과/275조건부 생략/실패·오류0. extractor114·패키지20·job PG196·migration18·정책부모 PG2·runtime5·worker PG12·Flyway3은 실패/오류/생략0이다. 선행 UI35972307990·관측35972899436도 success다.
- [ ] 다음은 HWPX 역할 구조 누락·부분 추출 원인 진단, PDF 구조 불확실 처리, 검토된 구간 기대값과 동일 버전 재검증이다. 옥천 승인 보류는 해제됐지만 전체9Gate·ATT62·SEG 확장 완료 기준은 그대로다. 브라우저는 현재 요청 정책상 미실행이다.

### 2026-09-24 전체 격리 QA 승인 및 새 보은 관측 경로

- 사용자가 모든 격리 QA 요청을 승인했다. 기존 제시한 대상·요청/byte·시간·자원 상한 안에서 순차 실행한다. 옥천 고정3건 보류도 해제하되 운영 정책/DB/worker 활성화·기존 데이터 적용·배포·브라우저 승인으로 확대하지 않는다.
- [x] AWS default/root·서울·저장소 계정 일치, 대상1대/SSM Online, 운영9a1bb45를 현재 재조회했다. 사전 SSM9d3df389-9f19-4966-9fcc-c0bf80cdb827 Success: 가용 메모리929MiB·cgroup v2·필수 격리 도구·운영 JAR12985f8c… 일치다. 운영 DB 조회/쓰기는 없다.
- [x] 기존 BOEUN worker 모드는 운영 설치 QA/추출기를 사용하므로 새 버전 증거로 재사용하지 않는다. BOEUN_OBSERVATION을 별도 추가하여 임시 최신 QA/추출기·고정3공고·132요청/240MiB를 사용한다. 태백/옥천/기존 worker 모드와 상한을 보존했다.
- [x] 전체 보고서에서 source/형식/본문/파일/구간 지문을 검증한 뒤 전송 metadata는 모든 파일과 역할 순서·개수·분석 지문을 보존한다. 구조 진단/좌표 배열을 전송 요약에 중복하지 않아 SSM 한도 초과를 방지한다. 요약 hash는 요약 자체가 아닌 전체 분석의 지문임을 표시하며 기대값 자동 승인은 하지 않는다.
- [x] Java 관측/probe31/31·23초, 패키지20/20(선행32초), Python16/16·Node/Bash4/4·PowerShell 구문 검사 통과다. 이번 증분은 시험/실행기만 변경하며 production Java·migration은 변경하지 않았다.
- [x] 새 보은 및 옥천 실행 결과/제한 실측/운영 불변/자기 임시 자원 정리는 위 후속 실행 근거로 확인했다. 관측 통과와 정상 후보/정책 QA는 구분한다.
...
- [~] 기존 AG30을 보존하고 SG22를 더한 신규 엔진 정답52건을 구현했다. 분류 검사 저장/조회, 전체 QA coordinator와 게시 재검증이 엔진·suite·정확한 case 목록/순서·구간 규칙 hash에 결합된다. 신규 엔진에서 기존30건의 성공을 재사용하지 않는다. 원문/참고 근거를 결과/감사 로그에 복제하지 않는다.
- [x] 실제 seed의 SG-005 실패를 재현했다. 음성 표본 `지원대상: 소상공인`에는 보조어 `지원`이 같은 문단에 있어 조합이 성립했다. 음성 입력을 `신청자격: 소상공인`으로 정정하고 보조어 단위 회귀를 추가했다. 판정 엔진·규칙 seed·기존 migration은 변경하지 않았으며 실패 case ID만 오류에 표시한다.
- [x] 표적 정책 회귀16suite286건, 실제 임시 PostgreSQL 저장/조회·멱등성/미게시1건(3.402초), bootJar를1분24초에 통과했다. 실패/오류/생략0이며 DRAFT 유지·job0·원문 없는 감사 metadata를 확인했다.
- [x] 전체 로컬 회귀5분51초 성공: root245suite2837건 중2564통과/273조건부 생략/실패·오류0. extractor90건과 bootJar는 선행 결과 UP-TO-DATE 재사용이다. Java·임시 PostgreSQL0 및 단기 Node 종료, 기존 migration 변경0·사용자 output 보존을 확인했다.
- [~] 새 커밋 Linux 검증은 별도로 확인한다. 실제 Provider 구간 기대값·PDF 문단/표 구조·관리자 구간 UI·실파일 QA는 남는다. 정책 기본 엔진 전환/운영 활성화/기존 데이터/옥천 QA/브라우저는 실행하지 않았다. 전체 Gate8진행/1차단, 목표 active를 유지한다.

### 2026-09-22 구간 엔진 worker 연결 및 V85 검증 진행

- [~] `attachment-segment-1.0.0`/`segment-role-1.0.0`/규칙 hash가 함께 고정된 작업만 새 엔진으로 실행하도록 예약·수집·재시도·배치·역할 재판정·worker를 연결했다. 기존 미설정 정책의 JSON/manifest/판정 해시는 유지한다. 새 manifest는 schemaVersion3이며 평가 입력·판정 해시에 구간 근거를 포함한다.
...
- [x] 구간별 분석 설계·구현 승인을 받았다. [새 버전 설계](announcement-attachment-segment-analysis-design-2026-09-22.md)의 SEG-001~010을 추가 관리한다. 기존 파일 역할/봉인 이력, 제목 제외/A·B/최종 관리자 검증/자동 활성화 금지는 보존한다.
- [~] 독립 구간 분석기, 별도 버전의 구간 종합 엔진, additive V84, SHADOW 분석 저장·조회 API와 테스트를 구현·검증 중이다. 실제 worker/정책/manifest/UI에 아직 연결하지 않았다.
- [x] 로컬 전체 애플리케이션2549통과/270조건부 생략·추출기90통과·실패0, 최종 대상 재검증252통과/실제 DB4생략 및 bootJar 성공. Linux/V84 검증과 구간별 실파일 검증은 이 결과에 포함되지 않는다.
- [ ] PDF 문단·표 구조 추출은 별도 잔여 구현이다. 현재 PDF page block을 일괄 reliable로 바꾸지 않았으며 보은 PDF의 정상 판정을 주장하지 않는다.
- [!] 로컬 Docker daemon 접속 불가. 실제 PostgreSQL/V84 검증은 로컬 성공으로 기록하지 않는다. 기존 ATT-053 V83 완료 증거는 보존하되 V84 추가 검증을 완료 조건에 포함한다.
- e365fd8의 [Linux35695655986](https://github.com/FrostyCityMan/saneB/actions/runs/35695655986)은 success로 완료되었다. 이번 구간 분석 변경을 포함하지 않는 이전 커밋의 결과다.
- 운영 정책 활성화·기존 데이터 재처리·옥천 보류 상태는 변경하지 않았다. 전체 Gate8진행/1차단을 유지하며 구간 확장 완료 또는 전체 goal 완료로 보고하지 않는다.
...
- 설치된 production 추출기로 경계 입력의 OCR_REQUIRED, 초과 입력의 LIMIT_EXCEEDED·텍스트/근거 없음, 이후 정상 HWP의 COMPLETE_TEXT와 매 입력 원본/작업 폴더 정리를 요구하는 Linux runtime 시험을 추가했다. 실제 Linux 통과 여부는 새 SHA CI 결과 확인 전까지 미확인이다. OS OOM killer·다른 모든 악성 입력·운영 JVM 생존을 입증하는 시험은 아니다.
- 첫 로컬 실행은 테스트 코드의 POI 생성자 오류로 실패했고 수정 후 표적 검증+bootJar가 30초 성공했다. 새 추출기 시험2/2, root 표적23=22통과/Windows의 Linux 전용1생략, Node 판정기10/10이다. runtime 보고서 최소5건과 실패/생략0을 요구한다.
- bootJar 직접 검사에서 운영 fixture12개·새 제한 시험 파일/생성기0개를 확인했다. 운영 runtime suite/version/정책 지문, main Java/리소스, migration은 변경하지 않았다. 옥천 서버 QA 보류·별도 코드 재배포 응답 대기·운영 정책/worker/기존 데이터 승인 경계를 유지하며 공식 사이트와 브라우저는 실행하지 않았다. 사용자 output은 보존한다.

### 2026-09-22 사용자 범위 결정 — 옥천 서울 서버 QA 보류

- 사용자가 옥천 고정3공고의 서울 임시 격리 QA를 **보류**했다. 해당 요청을 단순 승인 대기로 표현하거나 기존 태백 승인/장기 goal의 자동 계속 지시로 재개하지 않는다. 로컬 plan849a8592…의 uploaded=false/commandId 없음도 재확인했다. 업로드·SSM 제출·공식 사이트 요청은 없으며 준비 패키지는 로컬에 보존한다.
- 코드 재배포 재개는 별도 질문으로 분리됐으며 아직 응답이 없다. 첨부 정책 게시·ENFORCE·기존 데이터 적용 승인도 아니다. 운영 서버를 쓰지 않는 a884ac1 Linux CI35679213890은 아래처럼 성공으로 종료됐다. 전체 goal 정지나 완료로 처리하지 않는다.

### 2026-09-22 ATT-025/027 실제 Linux 자원 실패 검증 추가

- [Linux35679213890](https://github.com/FrostyCityMan/saneB/actions/runs/35679213890), SHA `a884ac1f80bc6e2632fcb808fc49394333ba6b2d`는 최종 success다. runtime XML4/4·실패/오류/생략0이며 새 heap 한도·비정상 종료/부모 재개/원본 정리 시험0.522초, 실제30초 timeout·관측 자식 종료/원본 정리/다음 추출 시험30.303초가 통과했다. 기존 환경/파일/망 canary1.379초·12fixture4.966초도 통과했다. 이는 합성 자식 JVM의 실제 launcher 검증이며 OS OOM killer·모든 악성 parser 입력·운영 설치 검증으로 확대하지 않는다.
- root2768=2499통과/269조건부 생략/실패·오류0이다. 추출기88·패키징20·job192·migration17·worker12·Flyway3·정책 부모2도 실패/생략0이며 독립PG221/221·미실행/container 실패0 및 독립/정책 정리 성공을 확인했다. 중복 suite를 고유 시험 수로 합산하지 않는다. `build/qa-results/run-35679213890-contracts/`에 XML을 보관했다. Linux 업무 코드 지문 d49cb22d272b66bd5d10078d33f6f698ddb89bef53b48faa72a60cafa6096780은 이전과 동일하다.
...
| 3 분류·정책 | [~] | catalog37참조/13기관/보관 기대값1/정상0·coverage false. 함안1건은 본문/HWP signature만 확인한 미승인 참조. 달성1.0.11은 그림/쪽번호 및 PDF 부분 추출, 남구44381은 추출 완전/구간 미확정으로 검수 유지. 보은 명시 구간1.0.4 단일 worker 저장도 UNKNOWN1을 유지한다. 전체 Provider/형식/정상 기대값·현행 지문 QA·정책 게시 미완료 |
...
| 6 자동·실파일 QA | [!] | 전체 격리 QA 승인과 기존 누적 한도를 유지한다. 보은1.0.4 실제 worker/임시DB/API, 달성1.0.11 전체PDF/HWP 관측, 함안 첫 참조 본문/HWP signature를 구분한다.60fdc93 CI36031713182 completed/success이며 이번 함안 증분 CI는 별도다. 차단 사유는 QA 승인 대기가 아니라 전체 Provider의 검토된 정상/형식 기대값 및 미확정·부분 추출 해소 부족이다. 달성/보은/남구는 잔여 요청만으로 같은 전체파일 실행 반복 불가 |
