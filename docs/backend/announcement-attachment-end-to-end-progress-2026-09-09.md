...
| 3 분류·정책 | [~] | 로컬 전용 BODY18(철원 marker 재확인 필요). catalog30참조/보관 기대값1/정상0이나 DNS 전송 코드 변경 후 태백 PROFILE_CHANGED로 현행 실행0이다. 이전 태백 HWPX2파일 고정 비교와9fe892c 제천 HWPX3파일 worker/DB/API 성공은 당시 코드 근거다. 새 코드 재관측·전체 Provider/형식/정상 기대값·정책 게시는 잔여 |
...
| 6 자동·실파일 QA | [!] | 8589016 Linux35055559668 계약·독립 PG·부모 성공. 보은 재검증35055669009는 BODY TIMEOUT2회/NETWORK_TIMEOUT·파일 미처리/JUnit3실패다. 후속 DNS 분류 수정본의 전체 로컬 회귀는 태백 기대값 지문 불일치1실패이며 assertion/catalog를 유지하고 재관측 경로를 준비한다. 이전 제천/태백 성공·부분 추출과 전체 Provider·정상/형식 기대값 잔여를 구분한다 |
