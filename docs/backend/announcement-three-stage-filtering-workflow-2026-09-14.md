...
2026-09-16 충주시 증분: [공식 본문·새올 첨부 모델](announcement-chungju-eminwon-profile-2026-09-16.md)을 연결했다. 구현7종/첨부 모델19/전용 본문18/형식3이며 전체 대상 지원 완료가 아니다. 격리 DRAFT 제목 음성2건은 후속 요청0, 양성1건은 본문523자·HWP1개 signature 통과다. 새 catalog30참조/기대값1/정상0을 유지하며 실제 Linux 추출·역할·정책 QA·운영 적용은 별도 검증한다. 아래 수량은 과거 증분 기록이다.

2026-09-15 철원군 증분: [철원군 본문·첨부 모델](announcement-cheorwon-bbs-profile-2026-09-15.md)을 연결했다. 로컬 엔진6/첨부 모델18/전용 본문 연결17이나 신규 기관의 실제 사이트 검증은 미완료다. catalog27참조/보관 기대값1이며, BBS 코드 변경으로 태백 구지문은 `PROFILE_CHANGED`/현재 실행0으로 유지한다. 파일/게시판/현재 공고번호와 redirect 소속을 검증하며 실제 파일 응답을 확인하기 전 MIME 예외를 추가하지 않는다. 아래 과거 수량·실증은 각 버전의 기록이다.

최신 QA 증분(2026-09-15 12:28 관측 후): 태백 전체2파일의 사전 기대값1건을 catalog v2에 연결했다. 총24항목/참조전용23/고정 기대값1이며 혼합 UNKNOWN+신청서 FORM을 유지한다. 정상 공고 수0·전체 정책 QA 미완료다. [관측·검토·재검증 경계](announcement-taebaek-fixed-qa-expectation-2026-09-15.md)를 따르며 아래 기대값0은 당시 기록이다. 제목→본문→첨부→관리자 순서와 운영 승인 경계는 변경하지 않는다.

최신 기관 증분: [양평군 BBS 본문·첨부 프로필](announcement-yangpyeong-bbs-profile-2026-09-15.md)까지 추가했다. 전용 본문16기관, 첨부 엔진6/등록 프로필17/추출 형식3이다. 양평3공고의 BODY와 지원 파일3개(HWPX1/PDF2)의 다운로드/signature를 확인했고 미지원 JPG/PNG2개는 전체 발견5개 분모에 남겼다. 혼합 형식2공고를 전체 분석 완료로 보지 않는다. catalog 참조24/실행 기대값0이며 정상 다중첨부/역할·정책 QA는 미완료다. 태백184816의 과거 역할1.0.1 Linux 관측은 본문431자·HWPX2개 완전 추출 후 UNKNOWN 사유를 보존한 REVIEW_REQUIRED였고 현재 profile/역할 버전의 증거가 아니다. 전체 정상 후보·운영 적용으로 계산하지 않는다. 아래 과거 수량은 각 증분 시점의 기록이다.
