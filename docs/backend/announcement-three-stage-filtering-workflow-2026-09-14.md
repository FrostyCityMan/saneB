...
### P3 문서 역할 자동 식별의 연결 지점 (미구현, 다음 증분)
...
이 목록은 연결 설계이며 새 역할 판정 코드·migration·운영 적용의 완료 증거가 아니다. 엔진·모델 수는 여전히 첨부6/등록12/추출3이며 이 증분의 명칭이나 파일 수를 새 수집 모델 수로 세지 않는다.
...
- [~] P3 명시적 탐색 영역 정제 및 실측 BBS3모델의 본문 전용 셀 선택·구조 변경 실패 처리 구현. 본문 클라이언트27건과 기관별 공식 공개 본문3건, 관련 Service/분류 회귀 및 로컬 전체 시험·bootJar 통과. 나머지 본문 모델 및 문서 역할 자동 식별 규칙·예외·공식 QA 기대값은 잔여.
