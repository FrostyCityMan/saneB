...
### 2026-09-24 내부 절·중복 표제 규칙의 명시 실행 연결

- 실파일 후보 대조에서 HWPX 미확정 구간 감소를 확인한 `segment-role-1.0.3`을 정확한 지문 `8b9fdd872f3eb9890146d6e360408204ff285f4b23977e07693834aceec66d43`과 함께 명시 선택 목록에 추가한다. 기존0/2와 진단용1의 의미, 기본값0, 원문·block 좌표, A/B·FORM 참고·미확정 검수 정책은 유지한다.
- Design Read: 관리자 초안 편집 R1, 게시 R2 분리. 기존 Thymeleaf/Bootstrap 폼의 네이티브 선택 항목만 추가한다. 버전/지문 대조, 변경 전후 요약, 수정 시 동의 해제, 실패 시 입력 보존, 권한·busy·stale 잠금은 유지한다. 신규 UI 의존성은 없다.
- API 생성/수정의 nullable `segmentRuleVersion`과 명시 GET `analysisVersion`은0/2/3을 허용한다. 생략 생성/독립 POST는0, 생략 수정은 현재 버전 유지, 생략 GET은0이다. 새 버전 GET은 저장된 해당 지문만 조회하며 다른 버전이나 새로운 분석을 대신 만들지 않는다.
- worker 실행 snapshot·평가 입력 FK·판정 결합 조회·Provider 기대값은 같은 버전/지문을 요구한다. 기존 이력·고정 작업·운영 정책을 자동 갱신하지 않는다. DB/Flyway/v1 변경 없음.
- 정답 검증은 기존52건 계약을 유지한다.3의 SG-002만 내부 신청안내 절과 연속 신청서 표제를 포함해 NOTICE/FORM·원문 좌표·참고 근거를 검증한다.0/2의 fixture와 결과는 보존한다. 전체 Provider QA나 실파일 저장 검증을 정답 검증으로 대체하지 않는다.
- 성공 기준: 명시3 선택·서버 지문 고정·생략 보존·오지문/다른 버전 반환 거부, QA 근거 STALE, 버전3의 worker 저장과 평가 FK 불변. 실패 기준: 자동 승격/활성화, 다른 버전 분석 대체, UNKNOWN 정상화, 기존 원문 재분류·운영 설정 변경.
- 현재 범위는 로컬 연결 및 검증이다. 이전 서울 관측은 worker2 저장 + 후보3 메모리 대조였으며, 실제 worker3·DB/API 실파일 검증과 운영·브라우저 완료 근거로 재사용하지 않는다. 아래 항목은 각 증분 시점의 이력이다.

### 2026-09-24 정책 초안의 구간 규칙 선택

- 목적: 서버에 구현된1.0.2를 관리자가 명시적으로 선택하고 검증할 수 있게 한다. 기존 초안의 묵시적 승격·파일 엔진 변경·운영 정책 게시를 수행하지 않는다.
- Design Read: 기존 Thymeleaf/Bootstrap 정책 편집 폼·공통 CSS·영향 확인 영역을 재사용한다. 관리자 초안 편집 R1이며 최종 게시 R2는 기존 별도 확인을 유지한다. 한국어 라벨의 네이티브 select로 기존값 유지/1.0.0/1.0.2를 표시하고 사이트 parser 선택은 제공하지 않는다.
- 생성/수정 요청에 nullable `segmentRuleVersion`만 추가한다. 생성 생략은 기존 기본1.0.0, 수정 생략은 현재 버전 유지다. 알려진 정확한0/2만 허용하고 hash는 서버가 결정한다. 진단1.0.1·빈 문자열·공백·임의 지문은 거부한다. 기존 Java 생성자/JSON 생략 요청과 생성 멱등 hash는 호환 유지한다. 버전을 명시한 생성 요청은 그 버전도 멱등 identity에 포함한다.
- 수정은 DRAFT와 expectedVersion CAS를 통과해야 한다. ACTIVE/RETIRED와 기존 파일 단위 엔진의 구간 전환은 거부한다. 정책 row_version 증가와 policy_hash NULL을 기존 DAO로 처리하며 전체 QA/게시 준비는 현재 버전으로 재검증해야 한다. 기존 고정 작업과 source/evaluation을 갱신하지 않는다. V84/V85를 포함한 migration 변경은 없다.
- 화면은 현재 구간 버전/hash, 변경 전후 버전, 전체 QA 재실행 필요성을 표시한다. 입력 변경 시 동의 해제·QA/게시 잠금, 요청 실패 시 값 보존·동일 요청 재확인, 구 엔진/조회 역할 비활성화를 기존 흐름에 연결한다. 수정 성공 응답은 버전/hash까지 요청과 대조하며 다른 버전 반환을 성공으로 표시하지 않는다. 생성 재시도는 이후 편집된 현재 정책을 반환할 수 있으므로 생성 응답을 초기값으로 되돌리지 않는다.
- 성공 기준: 명시0↔2 선택/저장·조회·정답52건, 이전 QA의 STALE 판정, 기존 생략 요청 보존, 관리자 외 쓰기 및 임의 실행 설정 차단. 실패 기준: 자동 승격, 일반 파일 정책의 의미 변경, 이전 QA 재활용, 운영 게시/기존 데이터 적용이다. 브라우저·운영 E2E·전체 Provider 기대값 검증은 이 초안 기능의 로컬 시험과 구분한다.

### 2026-09-24 선택한 판정의 구간 근거 연결

- 사용자 과업: 운영자가 문서 전체를 반복 탐색하기 전에 선택한 판정에 실제 결합된 구간 역할·좌표와 남은 미확정 사유를 확인한다. 조회는 R0이나 잘못 연결된 근거로 검수할 위험을 우선 방지한다.
- Design Read: 기존 Thymeleaf/Bootstrap 검수 화면, `saneb-announcement-attachment-review.css`의 반응형·44px 버튼·포커스 표시와 공통 layout을 재사용한다. 새 라이브러리·화면 재설계는 없다. 주 행동은 판정 결합 근거 확인, 독립1.0.0 분석 조회는 별도 보조 행동이다.
- `GET .../segment-analysis/evaluations/{evaluationId}`는 판정→불변 입력→분석 ID→고정 정책 버전/hash를 대조한다. 기존 추출 접근 조건(원문·봉인 집합·최신 추출·PRODUCTION·제목 통과)을 유지하고 저장 JSON을 원문으로 재현 검증한다. 다른 버전 분석이 같은 추출에 존재해도 대체하지 않는다. 조회로 분석·재분류·검수·공개를 실행하지 않는다.
- 응답에는 선택한 판정 ID/정책 ID/판정 당시 파일 역할/현재 이력 여부와 기존 구간 응답을 제공한다. `evaluationCurrent`는 ENFORCE 적용이 아니며 과거 판정도 명시적으로 조회할 수 있다. 판정 시점 역할과 현재 파일 역할은 구분한다. 결합 없음404, 잘못된 결합409, 경로 ID 오류400을 한글과 no-store로 반환한다.
- 화면은 상단 요약의 집합·판정 또는 선택한 이력의 ID를 보존한다. 로딩·미분석·404/409·권한/세션/통신 오류와 동일 판정 GET 재시도를 제공하며 늦은 응답은 페이지/패널 세대가 바뀌면 폐기한다. 구간 확장은 네이티브 details, 원문 이동은 기존 문단 조회를 사용한다. 원문은 textContent로만 표시하며 입력/검수 확인 조건을 자동 변경하지 않는다.
- 성공 기준: 판정·source/set/file/extraction·분석 ID·버전/hash 일치, 원문 무복사, 조회 무변경, 과거/미리보기 비적용 표시, 기존 검수/DRAFT 잠금 보존. 실패 기준: 기본1.0.0으로 새 버전 근거 대체, UNKNOWN 또는 FORM을 정상 후보로 승격, 조회 실패를 빈 성공으로 표시, 무승인 운영 쓰기다.
- 검증: 서버 단위/권한 MockMvc, Node DOM 유사 객체 상호작용, 기존 버전별 실제 PG 시험2건에 결합 조회/잘못된 source·evaluation/독립 분석 공존 검사를 추가한다. 실제 브라우저·보조기술·레이아웃 및 검수 소요시간 감소는 아직 측정하지 않았으며 통과로 보고하지 않는다. 정책 버전 선택/게시와 전체 수집원 QA는 별도 잔여 업무다.

### 2026-09-24 분기 주표제 규칙의 명시 버전 실행 연결

후속 실파일 검증: BOEUN_SEGMENT 실행 fixture를 명시1.0.2로 전환하고 각10요청/24MiB로 축소했다. 이전 후보 관측의 고정 지문과 실제 저장 분석·평가 FK·버전 GET을 대조하며, 기본1.0.0 GET은 미저장 상태로 유지되어야 한다. 서울 임시 실행3/3이 통과했지만 UNKNOWN5/4/1과 관리자 원문 확인 요구는 남았다. [실행/정리/예산 근거](announcement-segment-seoul-qa-2026-09-24.md)의 dc11d4e 실행을 따른다. 기존 후보 메모리 대조와 실제 새 버전 worker 저장 검증을 구분한다.

- 목적: 실파일에서 확인한 `segment-role-1.0.2`를 기본값 교체 없이 worker/정책 QA에 연결한다. 기존 `1.0.0` 재현과 제목→본문→전체 첨부→관리자 최종 검증을 유지한다. 자동 활성화·규칙 게시·기존 데이터 재분류는 수행하지 않는다.
- EngineContract는 `1.0.0`/`1.0.2`의 정확한 버전·hash 쌍만 실행 가능으로 인정한다. 진단용 `1.0.1`은 worker 실행 대상이 아니다. 구간 엔진 이름은 `attachment-segment-1.0.0`이며 실행 snapshot의 구간 규칙이 의미를 고정한다.
- Evaluation Service는 snapshot의 규칙으로 분석·저장한다. 정책 초안의 일반 편집과 개정은 고정 버전을 보존하며 새 정책의 기본값은 `1.0.0`으로 유지한다. 신규 버전 선택/전환 UI는 이 서버 연결과 별도 잔여 작업이다.
- 분류 정답 세트 AG30+SG22는 각 정책의 구간 규칙으로 실행하고 결과 hash에 버전·hash를 결합한다. `1.0.2` SG-002는 정확한 분기 주표제/괄호 조건/신청서 분리를 실제 분석한다. `1.0.0` 고정 입력과 기존 파일 엔진30건은 보존한다.
- Provider QA는 기대값의 정확한 버전으로 재분석한다. catalog 기대값이 현재 정책의 구간 버전/hash와 다르면 `EXPECTATION_INVALID`로 분리한다. 구버전 기대값을 새 버전의 통과 근거로 재사용하거나 새 기대값을 자동 승인하지 않는다.
- GET `.../segment-analysis?analysisVersion=segment-role-1.0.2`는 해당 저장 결과만 반환한다. 생략 시 기존 `1.0.0`, 미저장 시 `NOT_ANALYZED`다. 잘못된 버전은400이며 분석 생성·다른 버전 fallback은 없다. 권한/no-store/ApiResponse를 유지하고 POST 수동 SHADOW 분석은 기존 `1.0.0`이다. `SHADOW`는 이 API 호출이 평가에 적용하지 않는다는 의미다.
- V84/V85의 버전별 유일성·정책/평가 불변 FK를 사용하므로 신규 migration은 없다. 합성 PG 시험은 동일 분기 문서에 대해 구버전 UNKNOWN/검수와 새 버전 NOTICE/후보를 구별하고, 두 분석 공존·정확한 조회·평가 FK 불변·공고 link 미생성을 검증한다. 실행하지 않은 PG 시험은 성공으로 보고하지 않는다.
- 성공 기준: 버전 고정 경로/정답/기대값/조회 계약 일치, 구버전 회귀 유지, 신구 분석 공존과 기존 평가 불변 검증. 실패 기준: 기본값 묵시적 승격, 기대값 버전 혼합, GET에 의한 생성, UNKNOWN/부분 추출의 정상 후보 승격, 운영 데이터 변경이다. 실제 운영·브라우저 E2E와 전체 수집원 QA는 별도 Gate다.

## 1. 목적과 승인 경계

### 2026-09-24 내부 신청 절·인접 중복 제목 후보

- 고정 보은 HWPX2파일의 XML을 메모리에서 확인했다. 기존 binary hash가 일치하며 공고 내부 `3. 신청안내`와 공백만 다른 인접 동의서 제목이 관측됐다. XML 관측과 설치 추출 결과는 구분한다. 원문 파일/문장은 저장하지 않고 사전의 업무 단어·위치·문자 모양만 확인했다.
- 후보 `segment-role-1.0.3`은1.0.2 결과를 기반으로 한다. 이전 구간이 필수 구조를 모두 갖춘 NOTICE이고 다음 구간이 정확한 번호형 `신청안내`로 시작하며 ROLE_STRUCTURE_INCOMPLETE인 경우에만 공고 내부 절로 연결한다. 독립 안내·다른 사업명이 있는 안내·완전한 별도 GUIDE·앞쪽 NOTICE 부족·불확실한 구조는 합치지 않는다.
- 이전 구간의 비어 있지 않은 전체 내용이 다음 첫 제목과 가로 공백만 다른 동일 제목일 때만 인접 중복 제목을 묶어 전체 범위로 재판정한다. 사이에 다른 문장이 있거나 제목이 다르면 구간을 유지한다. 신청인·서명 등 필수 구조를 제거하지 않는다.
- 원본 text/blocks hash·전체 code point 범위·원본 block/scope를 보존한다. 내부 절의 A/B는 NOTICE 근거로 기존 우선순위를 따르고 FORM의 키워드는 CONTEXT_ONLY다. 문단 사이의 대상/지원 AND는 계속 금지한다. 서문 UNKNOWN·부분 추출·불명확 PDF를 정상으로 바꾸지 않는다.
- 기본1.0.0, 명시 운영 가능1.0.2 및 이력 재현성을 유지한다. 후보는 운영 정책/worker/API 선택 목록에 추가하지 않았다. 실파일 QA에서 동일1.0.2 저장 분석과 메모리 후보만 비교하며 게시/ENFORCE/기존 데이터 적용은 없다.
- BOEUN_SEGMENT 후속 대조 상한은 고정3공고·각5요청/24MiB(합계15요청/72MiB)다. 선행8회 서버 영수증과 수동 XML5회 상한을 합친116요청/80,104,904byte를 차감하고 시작한다. 전체 승인132요청/240MiB를 넘겨 재실행하지 않는다.
...
#### 2026-09-24 후속: 괄호형 조건 표제 후보 규칙

- 고정 보은 HWPX 두 파일에서 확인한 `❍(신청자격) 값`, `❍(지원내용) 값`, `❍(신청기간) 값` 형식을 `segment-role-1.0.1`에만 추가한다. 같은 조건 규칙의 지원대상/지원규모/접수기간 표현도 테스트한다. ❍ 이외 임의 기호나 일반 문장 부분 일치로 확대하지 않는다.
- 정확한 버전+hash를 지정해야 새 규칙을 사용한다. 기존 기본 `segment-role-1.0.0`과 hash, 파일 역할 규칙은 변경하지 않는다. 기존/신규 저장 분석의 재현 검증은 해당 분석의 버전/hash를 사용하며 교차 결합을 거부한다.
- 동일한 신뢰 가능한 block 안의 한 줄 전체, 닫힌 괄호, 한 글자 이상의 문자/숫자가 있는 값만 근거 후보가 된다. 앞선 표제·나머지 필수 조건·원문 전체 범위·UNKNOWN/부분 추출/불확실 PDF 요건은 유지한다.
- 현재는 분석기 명시 호출과 격리 실파일 대조만 연결한다. `AttachmentEngineContract`는 아직 새 버전을 worker/정책에 허용하지 않는다. 일반 API와 DB schema는 그대로다. 후속 연결 시 고정된 과거 정책/작업의 버전 재현과 API 조회 기준을 먼저 보존해야 한다.
- BOEUN_SEGMENT의 `candidateSegmentComparison`은 동일 추출 결과를 메모리에서 비교한 후보 분석이다. `persistedOrApplied=false`이며 기존 worker DB/API 결과와 별개다. 원문/locator 없이 버전·hash·상태·구간 역할/근거 코드만 보고한다. 이 대조를 신규 정책 활성화/검토된 기대값 승인/정상 후보 증가로 해석하지 않는다.

#### 2026-09-24 후속: 분기 괄호가 붙은 공고 주표제

- 실제 설치 추출 결과의 조건 표제는1.0.1 문법에 맞지만 앞쪽0번 구간에 있었다. 뒤쪽 안내 구간에 조건을 복사하거나 구간을 합치지 않는다. 알려진 binary hash의 공개 XML에서 주표제가 `모집 공고(3분기)`로 끝나는 것을 확인했다.
- `segment-role-1.0.2` 명시 호출은 기존 표제와 `공고`/`공고문`/`모집 요강` 뒤의 정확한 `(1분기)`~`(4분기)` 표제만 인식한다. 한 줄/신뢰 block/길이/원문 순서·좌표/모든 필수 조건을 유지한다. 임의 괄호/문장 뒤 언급/잘못된 분기는 거부한다.
- 새 주표제를 경계로 추가하므로1.0.1과 구간 수가 달라질 수 있다. 표제 앞 원문은 UNKNOWN으로 보존하고 뒤쪽 안내·FORM 경계는 유지한다. 다른 구간의 조건으로 빈 안내 구간을 확정하지 않는다.
- 기본/worker/정책은1.0.0 그대로다. 1.0.0·1.0.1 규칙 hash와 재현성은 유지한다. 후보1.0.2도 운영 엔진 계약에는 아직 허용하지 않는다. API shape·migration 변경은 없다.
- 실파일 probe의 `quarterHeadingComparison`은 기존 분석 불변·같은 text/blocks·원문 전체 범위·원본 block 내 evidence 좌표·신규 분석 재현성을 검사한다. 저장/적용false를 유지하고 버전/hash/역할/NOTICE 근거 수만 보고한다.
- 진단 `sectionLayout`은 사전에 있는6개 표제, code point 위치, 줄 길이, block/구간 번호, 신뢰도와 전체 줄 문법 일치 여부를 기록한다. 본문 값/locator/개인정보는 포함하지 않으며64행을 넘으면 실패한다. 이 좌표 metadata를 업무 API/감사 로그로 복제하지 않는다.
- 누적 승인 예산을 고려하여 최신 BOEUN_SEGMENT 상한을 공고당14요청/24MiB, 지역42요청/72MiB로 낮췄다. 이전20요청/32MiB 영수증은 당시 값으로 보존하며 새 한도로 과거를 재해석하지 않는다.
...
### 2026-09-24 최종 검수의 구간 근거 연결 보완

- 기존 검수 판정은 구간 엔진에서 모두 확정된 혼합 문서에도 파일 전체 UNKNOWN을 이유로 원문 전체 수동 확인을 다시 요구했다. 구간별 판정으로 검수 부담을 줄이는 목적과 불일치하므로 이 중복 요구를 보완한다.
- 검수 ServiceImpl→Evaluation DAO→Mapper는 현재 evaluation input에 실제 결합된 분석만 읽는다. source/set/file/extraction과 정책의 segment 버전/hash가 모두 일치해야 하며 별도 SHADOW 분석으로 대체하지 않는다. 새 테이블/컬럼은 없고 V84/V85 불변 결합을 사용한다.
- 모든 COMPLETE_TEXT의 전체 원문/block으로 분석을 재현한다. RESOLVED이고 UNKNOWN 구간이 없을 때만 TEXT_RULE/UNKNOWN 출처의 파일 전체 UNKNOWN에 대한 중복 수동 요구를 제거한다. 파일 역할 자체나 평가 이력은 수정하지 않는다.
- MANUAL/PROFILE 고정 역할과 미확정 구간, 부분/실패 파일, A/B·문맥·발견 경고를 보존한다. 누락·다른 추출·잘못된 정책 결합·변조 근거는 구체적409이며 빈 체크리스트가 아니다.
- EXTRACTED_TEXT는 준비된 근거를 관리자가 확인하는 방법이지 자동 승인이 아니다. 최종 confirmation·버전/멱등성 검증과 DRAFT만 생성하는 경로는 유지한다. 실제 미확정 보은/옥천 표본의 검수 요구를 해제하는 변경이 아니다.
- 검증은 정상 GUIDE/FORM 혼합·UNKNOWN 앞부분·부분 추출·A/B·고정 역할·근거 변조 단위 시험과 실제 임시 PostgreSQL의 확인→DRAFT/멱등성·자동 활성화 금지로 구분한다. 후자의 실행 증거가 없으면 완료로 기록하지 않는다.

### 실파일 검수 컨텍스트 검증

BOEUN_SEGMENT의 실제 worker 결과로 관리자 `review-context` Service/GET 응답도 대조한다. UNKNOWN 구간은 원문 확인 및 ATTACHMENT_ROLE_UNKNOWN을 요구해야 한다. no-store·다른 source404·반복 GET의 동일 버전·확인 및 공고 link0을 검사한다. 보고서는 검증 여부와 수동 확인 필요 여부만 추가하며 원문이나 검수 메모를 출력하지 않는다. 새 probe는 이 근거가 없는 과거 보고서를 현재 성공으로 인정하지 않는다. 고정3공고·각20요청/32MiB 상한은 그대로다. 이 읽기 경로 검증은 관리자 최종 확인/DRAFT 생성이나 인증·브라우저 E2E를 대신하지 않는다.

### 버전 고정 판정과 최종 확인의 통합 검증

1.0.2로 RESOLVED인 분기 공고/FORM 합성 문서에 대해, 관리자 EXTRACTED_TEXT 확인 뒤 별도의1.0.0 SHADOW 분석을 추가해도 현재 판정에 결합된 분석과 확인 버전은 바뀌지 않아야 한다. 확인 이전 버전으로 DRAFT 전환은 거부하고 최신 확인으로만 단건 DRAFT·확정 다중 분류를 생성한다. 재시도는 같은 결과이며 자동 승인 요청은0이다. 동일 문서를1.0.0으로 판정한 경우 UNKNOWN·수동 확인 필요를 유지하고 EXTRACTED_TEXT 확인/미확인 전환을 거부한다. 실제 PostgreSQL 시험2건을 추가했으며 이번 SHA의 Linux 실행 전까지는 실행 대기다. 합성 DB 검증과 실제 파일·실사용자·운영 브라우저 검증을 구분한다.
