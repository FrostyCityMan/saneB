...
### 2026-09-25 긴 서명란 규칙의 명시 실행 연결

- 실파일 대조에서 동의서 UNKNOWN 1구간 개선을 확인한 `segment-role-1.0.4`를 정확한 지문 `27dfa69f39bea3c47e1bf40b20bf01471f2432bc08ee143355e4e849089406fe`과 함께 정책/worker/조회 계약에 연결한다. 진단1은 계속 선택 불가이며 기본0 및 기존0/2/3의 지문·규칙·고정 작업은 보존한다. 자동 정책 개정·게시·재분류·활성화는 없다.
- Design Read: 한국어 관리자 정책 초안 편집 R1과 운영 게시 R2를 분리한다. 기존 Thymeleaf/Bootstrap 네이티브 선택 상자에 한 항목만 추가하고 레이아웃·의존성은 유지한다. 선택→변경 전후 확인→초안 저장→새 QA의 기존 흐름과 입력 보존·동의 초기화·권한/busy/stale 잠금·저장 응답 지문 검사를 재사용한다. 게시의 영향 확인을 생략하지 않는다.
- API의 선택적 `segmentRuleVersion`/`analysisVersion`은0/2/3/4를 허용한다. 생략 생성·기본 GET·독립 POST는0, 생략 수정·개정은 현재 버전 유지다. 해당 버전의 저장 결과가 없으면 NOT_ANALYZED이며 다른 이력이나 새 분석으로 대체하지 않는다. DB는 기존 V84/V85의 버전/지문 유일성·평가 FK를 사용하며 migration/v1 변경은 없다.
- 정답 검증은 AG30+SG22=52건이다.4의 SG-002에 187자 신청인/서명란을 넣고 NOTICE/FORM 및 서식 A/B 참고 전용을 검사한다. 같은 입력의3은 NOTICE/UNKNOWN으로 남아야 한다.0/2/3의 정답 입력은 변경하지 않는다.
- 성공 기준: 명시4만 실행, 버전/지문 혼용 거부, 기본값 불변, 버전 변경 후 이전 QA stale, 저장 분석과 평가 FK/GET 결합, 관리자 최종 확인 후에만 멱등 DRAFT 전환. 실패 기준: UNKNOWN 정상 승격, A/B 정책·FORM 참고 경계 변경, 자동 공개 또는 기존 이력 재연결.
- 검증 계획: 표적 Java → 전체 첨부 회귀/패키지/bootJar → Node 화면 상태 → Linux 실제 임시 PostgreSQL. 현재 사용자 정책상 브라우저 검증은 미실행이다. 선행 보은 결과는 worker3 저장+후보4 메모리 비교이며 실제 worker4 저장 성공으로 대신하지 않는다. 잔여6요청의 기존 누적 예산은 유지한다.

### 2026-09-24 긴 신청인·복수 서명란 후보

- 기존 지문이 일치하는 보은 HWPX2개를 최대4요청/4MiB로 메모리 XML 관측했다.221497의 동의서 신청인/복수 서명란은 한 문단187자다. 현재 전체 줄 허용200자보다 작은 신청인 값160자/서명 앞부분120자 제한 때문에 같은 문단의 필수 필드가 누락될 수 있다. 실제 설치 추출 결과 개선은 별도 검증한다.
- 진단 후보 `segment-role-1.0.4`는1.0.3 구간 경계·원문·block을 그대로 사용한다. FORM_HEADING이 있고 ROLE_STRUCTURE_INCOMPLETE인 UNKNOWN 구간에만 신청인·서명 전체 줄 규칙을200자 상한 안에서 다시 적용한다. 새 표제 인식, 임의 서문 제거, 구간 병합, 파일/문단 간 근거 합성은 없다.
- 필수 신청인/서명·전체 줄 일치·단일 신뢰 block 조건을 유지한다.201자 이상·누락·문장 뒤 추가 텍스트·block 경계 초과·불확실 PDF·부분 추출은 해제하지 않는다. 확인된 FORM은 참고 전용이며 긍정/제외 근거로 승격하지 않는다.
- 기존0/1/2/3 hash와 기본값·worker·정책·API 선택 목록은 보존한다. 새 후보는 명시 메모리 분석만 허용하며 실파일 QA 후 별도 실행 계약 연결이 필요하다. DB/API/migration 변경 없음. 검수 시간 감소나 정상 후보 확보를 이 후보 구현만으로 선언하지 않는다.
