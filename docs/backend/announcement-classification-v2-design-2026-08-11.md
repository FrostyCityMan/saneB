...
> 2026-09-08 설계 확장: [첨부파일 수집·텍스트 추출 상세 설계](announcement-attachment-collection-design-2026-09-08.md)를 추가했다. PDF/HWP/HWPX 처리는 향후 확장 모드이며 현재 구현의 첨부 제외 계약을 바로 변경하지 않는다. 본 문서의 자동 제외 원문 보관에 관한 과거 잔여 문구는 V70 비저장 정책을 따른다. 운영 상태는 각 기록의 확인 일자를 기준으로 해석한다.
