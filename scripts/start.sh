...
# 동일 JAR의 불변 QA·추출기 패키지를 선택한다. app.env/실행 플래그/공용 설치는 변경하지 않는다.
# 명시된 경로는 각각 보존하며 정책 QA가 해당 설치 지문을 별도로 검증한다.
# 이전 웹 JAR 복구 후 재시작하면 기본 worker 추출기도 이전 release로 함께 돌아간다.
