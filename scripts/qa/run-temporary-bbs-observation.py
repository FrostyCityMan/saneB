...
SCOPES = {
    ...
    'BOEUN_SEGMENT': ('BOEUN-THREE-NOTICES', ['BOEUN-221499', 'BOEUN-221497', 'BOEUN-218812'], 42, 75497472),
}
        ...
        if mode=='BOEUN_SEGMENT' and report.get('status')=='PASSED':
            for case in cases:
                if (case.get('scope')!='OFFICIAL_WORKER_EPHEMERAL_DB_API_V1'
                        ...
                        or case.get('maximumRequestReservations')!=14 or case.get('maximumReservedBytes')!=25165824):raise ValueError('PROBE_OUTPUT_INVALID')
                for key,lower,upper in [('requestReservationsIncludingBodyUpperBound',3,14),('reservedBytesIncludingBodyUpperBound',1,25165824)]:
                    value=case.get(key)
