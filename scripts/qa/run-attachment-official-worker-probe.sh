...
# 공고당44요청/80MiB로 제한한다. 운영 home/env/socket은 공유하지 않으며 파일 추출은 별도 network 격리다.
...
if output="$(env -i PATH=/usr/bin:/bin LANG=C.UTF-8 \
  ...
  -cp '/probe.jar:/qa/lib/*' com.saneb.db.AnnouncementAttachmentOfficialWorkerProbe "$4" 2>/dev/null)"; then
  probe_exit=0
else
  probe_exit=$?
fi
