...
const cases = [['JECHEON-403587', 1], ['JECHEON-403530', 1], ['JECHEON-403490', 2]];
...
export function validateJecheonObservation(xml, reports, startedAtMs) {
  ...
  if (suite.tests !== 3 || !Array.isArray(reports) || reports.length !== 3
      || new Set(reports.map(r => r?.caseCode)).size !== 3) throw new Error('제천 고정 3공고의 전체 실행 증거가 필요합니다.');
  const summaries = [];
  for (const [code, listed] of cases) {
    ...
    if (!r || r.scope !== 'OFFICIAL_THREE_STAGE_OBSERVATION_V1' || !Number.isFinite(observed)
        ...
        || r.profileCode !== 'LOCAL_JECHEON_BBS_V1' || r.expectedListedFileCount !== listed
        ...
        || !Array.isArray(r.files)) throw new Error('제천 관측의 시각·범위·정리·예산 증거가 불완전합니다.');
    if (code === 'JECHEON-403587') {
    ...
    if (r.status !== 'OBSERVED_NOT_VALIDATED' || r.titleStage !== 'COMBINATION_MATCHED'
      ...
      throw new Error('제천 제목 통과 공고의 본문·전체 첨부·종합 판정 증거가 불완전합니다.');
    for (const f of r.files) {
      if (f?.status !== 'OBSERVED' || f.format !== 'HWPX' || !bounded(f.bytes, 1, 20971520)
          || !['COMPLETE_TEXT', 'PARTIAL_TEXT', 'OCR_REQUIRED', 'ENCRYPTED', 'CORRUPT', 'UNSUPPORTED', 'LIMIT_EXCEEDED'].includes(f.quality))
        throw new Error('제천 전체 파일 중 미실행·실패·형식 변경이 있습니다.');
  ...
  return { referenceCount: 3, tests: suite.tests, titleStoppedCount: 1, observedNoticeCount: 2,
